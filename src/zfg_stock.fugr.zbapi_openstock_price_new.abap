FUNCTION zbapi_openstock_price_new.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"  TABLES
*"      IT_OPEN_PRICE STRUCTURE  ZSTR_OPEN_PRICE_NEW
*"  EXCEPTIONS
*"      INCORRECT_PLANT
*"----------------------------------------------------------------------
...
*DATA: lt_outtab TYPE STANDARD TABLE OF alv_t_t2.
  ...
  DATA: lv_anfwert(09)   TYPE p DECIMALS 2.
...
****************get the plant details*************
  SELECT SINGLE * FROM t001w INTO @DATA(ls_t001w) WHERE werks = @plant.
...
*  ENDIF.
  ...
  LOOP AT lt_final INTO ls_final.
    ...
    ENDIF.
    ls_open_price-plant = ls_t001w-werks."ls_final-werks.
    ls_open_price-name1 = ls_t001w-name1."ls_final-name1.
    ls_open_price-start_date = ls_final-start_date.
    ls_open_price-to_date = ls_final-end_date.

    CLEAR: ls_final1,lv_lbkum, lv_lbkum1, lv_anfwert.
    LOOP AT lt_final1 INTO ls_final1 WHERE matnr = ls_final-matnr
      ...
      lv_anfwert = lv_anfwert + ls_final1-anfwert.
    ENDLOOP.

    ls_open_price-opening_stock = lv_lbkum.
    ls_open_price-closing_stock = lv_lbkum1.
    ls_open_price-open_stock_price = lv_anfwert.
    IF lv_anfwert GT 0.
      ls_open_price-unit_price = lv_anfwert / lv_lbkum.
    ENDIF.
