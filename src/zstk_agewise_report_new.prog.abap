...
REPORT  zstk_agewise_report_new.
...
DATA: gt_mara_bismt TYPE STANDARD TABLE OF ty_bismt.
...
*       PRICE TYPE MBEW-STPRS,              "Standard price 03/09/2014
         ...
         zage10        TYPE p DECIMALS 2,             " A5 Above 180 Days " A5 366-720 Days
         vzage10       TYPE p DECIMALS 2,            " Stock In Ltrs
         zage11        TYPE p DECIMALS 2,           " A5 721-1030 Days
         vzage11       TYPE p DECIMALS 2,           " Stock In Ltrs
         zage12        TYPE p DECIMALS 2,           " A5 > 1040 Days
...
DATA:gv_matnr type matnr18.    ""added by zakir
...
*-------code added by zakir-----------*
    SELECT
      ...
      WHERE matnr = @gt_mara-matnr INTO TABLE @gt_mara_bismt.
...
IF so_matnr is NOT INITIAL OR so_mtart is not INITIAL or s_charg is NOT INITIAL or so_spart is NOT INITIAL.
    LOOP AT gt_mara_bismt ASSIGNING FIELD-SYMBOL(<fs_data>) WHERE matnr NA SY-ABCDE and lvorm <> 'X'.
    unpack <fs_data>-bismt to lv_char18.
    <fs_data>-bismt = lv_char18.
       clear lv_char18.
    ENDLOOP.
      SELECT FROM mchb
      FIELDS matnr,werks,charg,ersda
      FOR ALL ENTRIES IN @gt_mara_bismt
      WHERE matnr = @gt_mara_bismt-bismt INTO TABLE @data(gt_mchb_bismt).

      SORT gt_mchb_bismt BY matnr werks charg.
...
*        SELECT SINGLE MIN( ersda )  FROM mchb INTO wa4 WHERE matnr = wa_mchb-matnr  AND charg = wa_mchb-charg.                      " Added by mani 13.02.2016
        ...
        CLEAR: wa_mchb_cop, wa4, gv_matnr.
*----------Code added by zakir---------------*
IF gt_mchb_cop IS NOT INITIAL.
...
**check record exists with old material and no deletion flagged-->Only new materials check for old material.
        ...
        ENDIF.
        READ TABLE gt_mchb_cop INTO wa_mchb_cop WITH KEY matnr = COND #( WHEN gv_matnr is not initial then gv_matnr else gs_mara_bismt-bismt )
                                                      werks = wa_mchb-werks charg = wa_mchb-charg BINARY SEARCH.
        IF sy-subrc = 0 and wa_mchb_cop-ersda is NOT INITIAL.
** Take old material's created date
          wa4-ersda4 = wa_mchb_cop-ersda.
          ELSE.
** if old material created date is empty , then take the new material date by batch
        READ TABLE gt_mchb INTO data(gs_mchb) WITH KEY matnr = gs_mara_bismt-bismt werks = wa_mchb-werks charg = wa_mchb-charg BINARY SEARCH.
        IF sy-subrc = 0.
         wa4-ersda4 = gs_mchb-ersda.
        ELSE.
          wa4-ersda4 = wa_mchb-ersda.
        ENDIF.
        ENDIF.
        ELSE.
*         READ TABLE gt_mchb_cop INTO wa_mchb_cop WITH KEY matnr = wa_mchb-matnr werks = wa_mchb-werks charg = wa_mchb-charg BINARY SEARCH.
*        IF sy-subrc = 0.
*          wa4-ersda4 = wa_mchb_cop-ersda.
*        ENDIF.
        wa4-ersda4 = wa_mchb-ersda.
...
ELSE.
**read Gt_mra_bismt with mchb-matnr
        READ TABLE gt_mara_bismt INTO gs_mara_bismt WITH KEY matnr = wa_mchb-matnr BINARY SEARCH.
**check record exists with old material and no deletion flagged-->Only new materials check for old material.
        IF Sy-subrc = 0 and gs_mara_bismt-bismt is not INITIAL and gs_mara_bismt-lvorm = ' '.
        IF gs_mara_bismt-bismt NA SY-ABCDE.
        UNPACK gs_mara_bismt-bismt to gv_matnr.
        ENDIF.
READ TABLE gt_mchb_bismt INTO data(wa_mchb_bismt) WITH KEY matnr = COND #( WHEN gv_matnr is not initial then gv_matnr else gs_mara_bismt-bismt )
                                                         werks = wa_mchb-werks charg = wa_mchb-charg BINARY SEARCH.
       IF sy-subrc = 0 and wa_mchb_bismt-ersda is NOT INITIAL.
** Take old material's created date
          wa4-ersda4 = wa_mchb_bismt-ersda.
          ELSE.
** if old material created date is empty , then take the new material date by batch
       READ TABLE gt_mchb INTO gs_mchb WITH KEY matnr = gs_mara_bismt-bismt werks = wa_mchb-werks charg = wa_mchb-charg BINARY SEARCH.
        IF sy-subrc = 0.
          wa4-ersda4 = gs_mchb-ersda.
          ELSE.
           wa4-ersda4 = wa_mchb-ersda.
        ...
        ENDIF.
        ELSE.
*         READ TABLE gt_mchb_bismt INTO wa_mchb_bismt WITH KEY matnr = wa_mchb-matnr  werks = wa_mchb-werks charg = wa_mchb-charg BINARY SEARCH.
*        IF sy-subrc = 0.
*          wa4-ersda4 = wa_mchb_bismt-ersda.
*        ENDIF.
        wa4-ersda4 = wa_mchb-ersda.
...
*-------------------------------------------------------*
        ...
        wa_final-old_material = gs_mara_bismt-bismt.
        ...
        ELSEIF t_days BETWEEN  181 AND 365."t_days > 180.
          wa_final-zage9 = wa_mchb-clabs.
        ELSEIF t_days BETWEEN 366  AND 730.
          wa_final-zage10 =  wa_mchb-clabs.
        ELSEIF t_days BETWEEN 731  AND 1095.
          wa_final-zage11 =  wa_mchb-clabs.
        ELSEIF t_days > 1095.
          wa_final-zage12 =  wa_mchb-clabs.
        ENDIF.
        ...
        wa_final-vzage10 = wa_final-zage10 * wa_final-volum.
        wa_final-vzage11 = wa_final-zage11 * wa_final-volum.
        wa_final-vzage12 = wa_final-zage12 * wa_final-volum.
        ...
        wa_final-stockvalue11 = wa_final-stockvalue1 * wa_final-vzage10.
        wa_final-stockvalue12 = wa_final-stockvalue1 * wa_final-vzage11.
        wa_final-stockvalue13 = wa_final-stockvalue1 * wa_final-vzage12.
        ...
        wa_final-stockcase10 = wa_final-price * wa_final-zage10.
        wa_final-stockcase11 = wa_final-price * wa_final-zage11.
        wa_final-stockcase12 = wa_final-price * wa_final-zage12.
...
*&---------------------------------------------------------------------*
  ...
  IF wi_batch = 'X'.
    ...
    PERFORM alv_layout USING 9 'Created On' 'ERSDA' 'GT_FINAL' '' '' ''.
...
*  PERFORM ALV_LAYOUT USING 48 '91-120 Stock Qty Value' 'STOCKVALUE8' 'GT_FINAL' 'X' 'X' 'X'.
    ...
    PERFORM alv_layout USING 51 '181-365  Days(Case)' 'ZAGE9' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 52 '181-365 Days Stock Value' 'STOCKCASE9' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 53 '181-365 Stock Qty Ltrs' 'VZAGE9' 'GT_FINAL' 'X' 'X' ''.

    PERFORM alv_layout USING 54 '366-730  Days(Case)' 'ZAGE10' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 55 '366-730 Days Stock Value' 'STOCKCASE10' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 56 '366-730 Stock Qty Ltrs' 'VZAGE10' 'GT_FINAL' 'X' 'X' ''.

    PERFORM alv_layout USING 57 '731-1095  Days(Case)' 'ZAGE11' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 58 '731-1095 Days Stock Value' 'STOCKCASE11' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 59 '731-1095 Stock Qty Ltrs' 'VZAGE11' 'GT_FINAL' 'X' 'X' ''.

    PERFORM alv_layout USING 60 '> 1095  Days(Case)' 'ZAGE12' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 61 '> 1095 Days Stock Value' 'STOCKCASE12' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 62 '> 1095 Stock Qty Ltrs' 'VZAGE12' 'GT_FINAL' 'X' 'X' ''.
*    PERFORM alv_layout USING 55 '> 180 Stock Qty Ltrs' 'VZAGE9' 'GT_FINAL' 'X' 'X' ''.
*    PERFORM alv_layout USING 56 '> 180 Stock Qty Ltrs' 'VZAGE9' 'GT_FINAL' 'X' 'X' ''.

    PERFORM alv_layout USING 63 'Old Material' 'OLD_MATERIAL' 'GT_FINAL' ' ' '' ''. "Added by zakir for old material column
****************************************  "Added By Govind On 27/11/2014
...
*  CLEAR WA_SORT.
  ...
  IF wo_batch = 'X'.                                           "changes on 24/09/014 by savariar
    ...
    PERFORM alv_layout USING 5 'Material Description' 'MAKTX' 'GT_FINAL' ' ' '' ''.
    PERFORM alv_layout USING 6 'Material Type' 'MTART' 'GT_FINAL' ' ' '' ''.
    PERFORM alv_layout USING 8 'Storage Location' 'MLGORT' 'GT_FINAL' ' ' '' ''. "  ADD  by sri 28/09/2016
...
***************************  "Added By Govind On 27/11/2014
    ...
    PERFORM alv_layout USING 47 '181-365  Days(Case)' 'ZAGE9' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 48 '181-365 Days Stock Value' 'STOCKCASE9' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 49 '181-365 Stock Qty Ltrs' 'VZAGE9' 'GT_FINAL' 'X' 'X' ''.

    PERFORM alv_layout USING 50 '366-730  Days(Case)' 'ZAGE10' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 51 '366-730 Days Stock Value' 'STOCKCASE10' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 52 '366-730 Stock Qty Ltrs' 'VZAGE10' 'GT_FINAL' 'X' 'X' ''.

    PERFORM alv_layout USING 53 '731-1095  Days(Case)' 'ZAGE11' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 54 '731-1095 Days Stock Value' 'STOCKCASE11' 'GT_FINAL' 'X' 'X' ''.
    PERFORM alv_layout USING 55 '731-1095 Stock Qty Ltrs' 'VZAGE11' 'GT_FINAL' 'X' 'X' ''.
