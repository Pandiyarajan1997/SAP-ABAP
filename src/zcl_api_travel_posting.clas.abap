class ZCL_API_TRAVEL_POSTING definition
  public
  final
  create public .
...
CLASS ZCL_API_TRAVEL_POSTING IMPLEMENTATION.
      ...
      LOOP AT gt_input ASSIGNING FIELD-SYMBOL(<fs_input>).
        ...
        IF <fs_input>-employee_no IS INITIAL.
          APPEND VALUE #( employee_no = <fs_input>-employee_no
                          type        = 'E'
                          msg         = |Employee Number Missing| ) TO gt_output.
          CONTINUE.
        ENDIF.
        ...
        IF <fs_input>-amount IS INITIAL.
          APPEND VALUE #( employee_no = <fs_input>-employee_no
                          type        = 'E'
                          msg         = |Amount Missing in input| ) TO gt_output.
          CONTINUE.
        ENDIF.
        ...
        IF <fs_input>-doc_date IS INITIAL.
          APPEND VALUE #( employee_no = <fs_input>-employee_no
                          type        = 'E'
                          msg         = |Doc_date Missing| ) TO gt_output.
          CONTINUE.
        ENDIF.
        ...
        IF <fs_input>-post_date IS INITIAL.
          APPEND VALUE #( employee_no = <fs_input>-employee_no
                          type        = 'E'
                          msg         = |Post_date Missing| ) TO gt_output.
          CONTINUE.
        ENDIF.
        ...
        IF <fs_input>-pymttype IS INITIAL.
          APPEND VALUE #( employee_no = <fs_input>-employee_no
                          type        = 'E'
                          msg         = |Payment type Missing| ) TO gt_output.
          CONTINUE.
        ENDIF.

        SELECT SINGLE pernr, bukrs, plans FROM pa0001 INTO @DATA(ls_pernr) WHERE pernr = @<fs_input>-employee_no
        ...
        ELSE.
          ...
          SELECT SINGLE objid, sobid FROM hrp1001 INTO @DATA(l_hrp1001_cp)
                                     WHERE otype = 'P' AND objid = @ls_pernr-pernr
...
**Added by Samsudeen M on 08.06.2023
                ...
                IF sy-subrc = 0.
                  APPEND VALUE #( employee_no = ls_pernr-pernr
                                  ...
                                  msg         = |Reference { lv_refrence } already available| ) TO gt_output.
                ELSE.
                  "Employee Vendors Costcenter Fetching
