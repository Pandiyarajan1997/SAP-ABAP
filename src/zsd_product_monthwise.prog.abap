...
SELECT-OPTIONS: SO_BUKRS FOR LV_BUKRS MODIF ID TB2 OBLIGATORY,
                ...
                SO_MVGR1 FOR W_MVGR1 MODIF ID TB2, " ADDED BY RAM
                SO_MATNR FOR W_MATNR MODIF ID TB2,
                SO_FKART FOR LV_FKART NO-DISPLAY. .
...
*  IF P_CB <> 'X'. " Added by Goivnd On 05/06/2014
  ...
  IF P_SR <> 'X' AND P_CI <> 'X' .                      " Added by mani 19.02.2016
...
*   VBRK~FKSTO <> 'X' AND VBRK~BUKRS IN SO_BUKRS AND VBRK~SPART IN SO_SPART AND  VBRK~FKART IN SO_FKART and
*    VBRK~FKART NOT IN ('S1','YBRE','S2','JEX','F1','F2', 'YLRE','YIRE').
...
*  DELETE ADJACENT DUPLICATES FROM IT_VBRK COMPARING VBELN MATNR.
    ...
    ELSEIF P_CI = 'X'.                                          " Added by mani 19.02.2016
...
*   VBRK~FKSTO <> 'X' AND VBRK~BUKRS IN SO_BUKRS AND VBRK~SPART IN SO_SPART AND VBRK~FKART NOT IN ('S1','YBRE','S2','JEX','F1','F2').
   VBRK~FKSTO = 'X' AND VBRK~BUKRS IN SO_BUKRS AND VBRK~SPART IN SO_SPART." AND  FKSTO = 'X'.



ELSEIF P_SR = 'X'.                                            " Added by mani 19.02.2016
  ...
  SELECT    VBRK~BUKRS "#EC CI_DB_OPERATION_OK[2768887] " Added by <IT-CAR Tool> during Code Remediation
            ...
            WHERE VBRK~FKDAT IN SO_FKDAT  AND MATNR IN SO_MATNR
            AND VBRP~VKBUR IN SO_VKBU2  AND
           VBRK~FKSTO <> 'X' AND VBRK~BUKRS IN SO_BUKRS AND VBRK~SPART IN SO_SPART
           and ( FKART = 'YBBR' OR FKART = 'YBRE' OR FKART = 'YIRE' OR FKART = 'YFRE' OR FKART = 'YBFS'
...
*      ENDIF.
*      ENDIF.
"COMMENTD ENDED BY RAM ON 4/7/16
...
" commented end by ram on 17/2/16 .
