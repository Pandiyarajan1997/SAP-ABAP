...
GT_KNVV1 TYPE TABLE OF TY_KNVV,   "ADDED BY RAM ON 21/11/2014
...
SORT IT_VBRK BY VBELN. " Added by <IT-CAR Tool> during Code Remediation
    ...
    ZSKU_VBRK~NET_AMOUNT
                    INTO TABLE IT_VBRPK
    FROM ZSKU_VBRP JOIN ZSKU_VBRK ON
       ZSKU_VBRP~VBELN = ZSKU_VBRK~VBELN AND
       ZSKU_VBRP~WERKS = ZSKU_VBRK~WERKS AND
       ZSKU_VBRP~SKU_ID = ZSKU_VBRK~SKU_ID AND
       ZSKU_VBRP~VKORG = ZSKU_VBRK~VKORG WHERE
...
SORT IT_VBRPK BY SKU_ID VBELN . " Added by <IT-CAR Tool> during Code Remediation
  DELETE ADJACENT DUPLICATES FROM IT_VBRPK COMPARING SKU_ID VBELN .
  ...
  ELSEIF P_CB = 'X'. " Added by Goivnd On 05/06/2014
    SELECT     VBRK~BUKRS "#EC CI_DB_OPERATION_OK[2768887] " Added by <IT-CAR Tool> during Code Remediation
             ...
             FROM VBRK JOIN VBRP ON VBRK~VBELN = VBRP~VBELN "#EC CI_DB_OPERATION_OK[2768887] " Added by <IT-CAR Tool> during Code Remediation
             WHERE VBRK~FKDAT IN SO_FKDAT AND VBRK~VBELN IN SO_VBELN
     AND VBRK~KUNAG IN SO_KUNAG AND VBRP~VKBUR IN SO_VKBU2 AND VBRK~BUKRS IN SO_BUKRS  AND VBRK~SPART IN SO_SPART AND VBRK~FKART IN SO_FKART AND FKSTO = 'X' AND  VBRK~KDGRP <> '09'. .
SORT IT_VBRK BY VBELN . " Added by <IT-CAR Tool> during Code Remediation
    DELETE ADJACENT DUPLICATES FROM IT_VBRK COMPARING VBELN .
...
IF IT_VBRPK[] IS NOT INITIAL .
 SELECT KUNNR VKORG KDGRP SPART
    FROM KNVV INTO CORRESPONDING FIELDS OF TABLE GT_KNVV1
    FOR ALL ENTRIES IN IT_VBRPK WHERE KUNNR = IT_VBRPK-KUNNR AND SPART = IT_VBRPK-DIVISION AND VKORG = IT_VBRPK-VKORG AND  KUNNR IN SO_KUNAG.  "Added By Govind On 06/12/2014.


     SELECT VBELN POSNR FKIMG LTR AS VOLUM MATNR BASE_AMOUNT AS NETWR
      FROM ZSKU_VBRP INTO  CORRESPONDING FIELDS OF TABLE IT_VBRP1
       FOR ALL ENTRIES IN IT_VBRPK WHERE VBELN = IT_VBRPK-VBELN AND  SKU_ID = IT_VBRPK-SKU_ID . "AND  WERKS = IT_VBRPK-WERKS . "Added By Govind On 27/06/2015.
***************************************************
   APPEND LINES OF GT_KNVV1 TO GT_KNVV .
   APPEND LINES OF IT_VBRP1 TO IT_VBRP.
*************************************************
   ENDIF.
