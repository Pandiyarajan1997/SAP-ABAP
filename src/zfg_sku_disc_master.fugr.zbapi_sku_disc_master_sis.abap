FUNCTION ZBAPI_SKU_DISC_MASTER_SIS .
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
TYPES : BEGIN OF ty_cdhdr,
  ...
  END OF ty_cdhdr.
    ...
    DATA : it_CDHDR TYPE TABLE OF ty_CDHDR,
        ...
        DATA : it_005 TYPE TABLE OF ty_CDHDR,
            wa_005 TYPE ty_CDHDR.
            DATA : it_837 TYPE TABLE OF ty_CDHDR,
            wa_837 TYPE ty_CDHDR.

                    DATA : it_832 TYPE TABLE OF ty_CDHDR,
            wa_832 TYPE ty_CDHDR.

    data : it_konh TYPE TABLE OF konh, "#EC CI_USAGE_OK[2220005] " Added by <IT-CAR Tool> during Code Remediation
           wa_konh TYPE konh. "#EC CI_USAGE_OK[2220005] " Added by <IT-CAR Tool> during Code Remediation

if S_DATE[] is INITIAL.
    SELECT OBJECTID from CDHDR INTO TABLE it_CDHDR WHERE OBJECTCLAS = 'COND_A' and UDATE = sy-datum and tcode in ('VK11' , 'VK12').
else.
  SELECT OBJECTID from CDHDR INTO TABLE it_CDHDR WHERE OBJECTCLAS = 'COND_A' and UDATE in s_date and tcode in ('VK11' , 'VK12').
endif.

if it_cdhdr[] is NOT INITIAL.
 SELECT * from konh INTO TABLE it_konh FOR ALL ENTRIES IN IT_CDHDR WHERE KNUMH = IT_CDHDR-knumh AND   KSCHL IN ('Z004', 'Z005', 'Z006', 'ZPRB', 'ZBSD'). "#EC CI_USAGE_OK[2220005] " Added by <IT-CAR Tool> during Code Remediation
 endif.


 loop at it_konh INTO wa_konh WHERE KVEWE = 'A' and KOTABNR = '5'.
   MOVE : wa_konh-knumh to wa_005-knumh.
   APPEND wa_005 to it_005.
   CLEAR : wa_005 , wa_konh.
 ENDLOOP.

 loop at it_konh INTO wa_konh WHERE KVEWE = 'A' and KOTABNR = '832'.
   MOVE : wa_konh-knumh to wa_832-knumh.
   APPEND wa_832 to it_832.
   CLEAR : wa_832 , wa_konh.
 ENDLOOP.


  loop at it_konh INTO wa_konh WHERE KVEWE = 'A' and KOTABNR = '837'.
   MOVE : wa_konh-knumh to wa_837-knumh.
   APPEND wa_837 to it_837.
   CLEAR : wa_837 , wa_konh.
 ENDLOOP.

if it_005[] is NOT INITIAL.
    SELECT KAPPL
           KSCHL
           VKORG
           VTWEG
        ...
        FROM A005
        FOR ALL ENTRIES IN IT_005
        WHERE KAPPL = 'V'
        AND   KSCHL IN ('Z004', 'Z005', 'Z006', 'ZPRB', 'ZBSD')
        AND   VKORG IN R_VKORG
       and KNUMH = IT_005-knumh ORDER BY PRIMARY KEY.  " Added by <IT-CAR Tool> during Code Remediation
...
* DELETE IT_A005_TEMP1 WHERE KSCHL <> 'Z005' AND KSCHL <> 'Z004' AND KSCHL <> 'Z006' AND KSCHL <> 'ZPRB' AND KSCHL <> 'ZBSD'.
...
*          AND    B~ERDAT IN S_DATE.
    ...
    if IT_A005_TEMP[] is NOT INITIAL.  " added on 19.11.2016
    SELECT VKORG
           VKBUR
      FROM TVKBZ
      INTO TABLE IT_VKBUR FOR ALL ENTRIES IN IT_A005_TEMP " added for all enteries on 17.11.2016
      WHERE VKORG eq IT_A005_TEMP-VKORG.
endif. "added on 17.11.2016
    IF SY-SUBRC = 0.
...
if it_832[] is not INITIAL.
    SELECT A~KAPPL
      ...
      FOR ALL ENTRIES IN it_832 "IT_MARA
      WHERE A~KAPPL = 'V'
       AND  A~KSCHL IN ('Z004', 'Z005', 'Z006', 'ZPRB', 'ZBSD')
      AND   A~KNUMH = it_832-KNUMH.  "A~MATNR  = IT_MARA-MATNR.
*      AND    C~ERDAT IN S_DATE.

* DELETE IT_A832 WHERE KSCHL <> 'Z005' AND KSCHL <> 'Z004' AND KSCHL <> 'Z006' AND KSCHL <> 'ZPRB' AND KSCHL <> 'ZBSD'.

 ENDIF.
*--------------------------------------- Fetch A837 - Sales Office/Material

 if it_837[] is not INITIAL.
    SELECT A~KAPPL
           A~KSCHL
           A~VKBUR
           A~MATNR
      ...
      FOR ALL ENTRIES IN it_837
      WHERE A~KAPPL = 'V'
       AND  A~KSCHL IN ('Z004', 'Z005', 'Z006', 'ZPRB', 'ZBSD')
*      AND   A~MATNR  = IT_MARA-MATNR
      AND   A~VKBUR IN R_VKBUR
      AND    A~KNUMH = it_837-KNUMH.

* DELETE IT_A837 WHERE KSCHL <> 'Z005' AND KSCHL <> 'Z004' AND KSCHL <> 'Z006' AND KSCHL <> 'ZPRB' AND KSCHL <> 'ZBSD'.
endif.
