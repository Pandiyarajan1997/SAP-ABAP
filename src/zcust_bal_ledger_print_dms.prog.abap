...
**********************Excess payment removal process*************
      SELECT * FROM zdms_dz_removal INTO TABLE @DATA(lt_delete)
                                    ...
                                    AND   dealer      = @p_reta.
      IF sy-subrc = 0.
*        LOOP AT lt_delete ASSIGNING FIELD-SYMBOL(<fs_delete>).
...
*remove the rounding off entry from bsid
      LOOP AT gt_bsid INTO wa_bsid WHERE ( blart = 'DR' OR blart = 'DG') AND budat = '20240331'.
        CLEAR lv_index. lv_index = sy-tabix.

        SELECT SINGLE * FROM faglflexa INTO @DATA(ls_flexa) WHERE ryear = @wa_bsid-gjahr AND docnr = @wa_bsid-belnr
           AND rldnr = '0L' AND rbukrs = 'DMS1' AND racct = '0042002009' AND rbusa = @lv_gsber.
        IF sy-subrc = 0.
          DELETE gt_bsid INDEX lv_index.
        ENDIF.
      ENDLOOP.
*remove the rounding off entry from bsid1
      LOOP AT gt_bsid1 INTO wa_bsid WHERE ( blart = 'DR' OR blart = 'DG') AND budat = '20240331'.
        CLEAR lv_index. lv_index = sy-tabix.

        SELECT SINGLE * FROM faglflexa INTO @ls_flexa WHERE ryear = @wa_bsid1-gjahr AND docnr = @wa_bsid1-belnr
           AND rldnr = '0L' AND rbukrs = 'DMS1' AND racct = '0042002009' AND rbusa = @lv_gsber.
        IF sy-subrc = 0.
          DELETE gt_bsid1 INDEX lv_index.
        ENDIF.
      ENDLOOP.

*remove the rounding off entry from bsad
      LOOP AT gt_bsad INTO wa_bsad WHERE ( blart = 'DR' OR blart = 'DG') AND budat = '20240331'.
        CLEAR lv_index. lv_index = sy-tabix.

        SELECT SINGLE * FROM faglflexa INTO @ls_flexa WHERE ryear = @wa_bsad-gjahr AND docnr = @wa_bsad-belnr
           AND rldnr = '0L' AND rbukrs = 'DMS1' AND racct = '0042002009' AND rbusa = @lv_gsber.
        IF sy-subrc = 0.
          DELETE gt_bsad INDEX lv_index.
        ENDIF.
      ENDLOOP.
*remove the rounding off entry from bsad1
      LOOP AT gt_bsad1 INTO wa_bsad1 WHERE ( blart = 'DR' OR blart = 'DG') AND budat = '20240331'.
        CLEAR lv_index. lv_index = sy-tabix.

        SELECT SINGLE * FROM faglflexa INTO @ls_flexa WHERE ryear = @wa_bsad1-gjahr AND docnr = @wa_bsad1-belnr
           AND rldnr = '0L' AND rbukrs = 'DMS1' AND racct = '0042002009' AND rbusa = @lv_gsber.
        IF sy-subrc = 0.
          DELETE gt_bsad1 INDEX lv_index.
        ENDIF.
      ENDLOOP.
...
* to remove the round off adjustment entry from BSID and BSAD tables created on march 31st
