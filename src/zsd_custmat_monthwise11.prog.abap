...
TYPES: BEGIN OF TY_VBRK_VBRP,                           "FINAL TABLE TYPE FOR OVERVIEW REPORT
       ...
       KDGRP TYPE VBRK-KDGRP,        "Added by savariar s
...
GT1_KNA1 TYPE TABLE OF TY_KNA1,
...
***""""""""""""" SAVARIAR SALES PRO TABLE********Starting************************
...
DATA: GT_SKUFINOVR TYPE STANDARD TABLE OF  TY_FINOVR,
      WA_SKUFINOVR TYPE                    TY_FINOVR,

      GT_ZVBRKP TYPE TABLE OF TY_ZVBRKP,
...
SELECT-OPTIONS: SO_BUKRS FOR LV_BUKRS MODIF ID TB2 OBLIGATORY NO INTERVALS,
...
*            VBRK~NETWR
   ...
   VBRK~FKSTO <> 'X' AND VBRK~BUKRS IN SO_BUKRS AND VBRK~SPART IN SO_SPART AND VBRK~FKART NOT IN ('S1','YBRE','S2','JEX','F1','F2') AND VBRK~KDGRP <> '09'.

*************** ***************Select Query Date for Sales Pro *********************************************************************
...
*          ZSKU_VBRK~NET_AMOUNT
     ...
     FROM ZSKU_VBRK
       INNER JOIN ZSKU_VBRP
            ON ( ZSKU_VBRK~VBELN = ZSKU_VBRP~VBELN AND
               ZSKU_VBRK~VKORG = ZSKU_VBRP~VKORG AND
               ZSKU_VBRK~WERKS = ZSKU_VBRP~WERKS AND
               ZSKU_VBRK~SKU_ID = ZSKU_VBRP~SKU_ID AND
               ZSKU_VBRK~DOC_YEAR = ZSKU_VBRP~DOC_YEAR )
     WHERE ZSKU_VBRK~VKORG IN SO_BUKRS
           AND ZSKU_VBRK~SKU_ID IN SO_VKBU2
           AND ZSKU_VBRK~KUNNR IN SO_KUNNR
...
*           AND ZSKU_VBRK~VBELN IN SO_VBELN
  ...
  IF IT_VBRK IS NOT INITIAL.
    ...
    SELECT
          MATNR
          VOLUM
          VOLEH FROM MARA INTO  TABLE IT_MARA FOR ALL ENTRIES IN IT_VBRK WHERE MATNR = IT_VBRK-MATNR  .

  ENDIF.

  "***************************Sales Pro Customers**Starting*********************************************

  IF GT_ZVBRKP[] IS NOT INITIAL.

    SELECT
    MATNR
    VOLUM
    VOLEH FROM MARA INTO  TABLE IT_MARA1 FOR ALL ENTRIES IN GT_ZVBRKP WHERE MATNR = GT_ZVBRKP-MATNR  .

    APPEND LINES OF IT_MARA1 TO IT_MARA.
  ...
  IF GT_ZVBRKP[] IS NOT INITIAL.
    ...
    SELECT  KUNNR
           NAME1
           NAME2
           REGIO FROM KNA1 INTO TABLE GT1_KNA1 FOR ALL ENTRIES IN GT_ZVBRKP WHERE KUNNR = GT_ZVBRKP-KUNNR.

    APPEND LINES OF GT1_KNA1 TO GT_KNA1.
  ...
  IF GT_ZVBRKP[] IS NOT INITIAL.
    ...
    LOOP AT GT_ZVBRKP INTO  WA_ZVBRKP.
      ...
      WA_VBRK-MATNR = WA_ZVBRKP-MATNR.

*     SHIFT WA_VBRK-KUNRG LEFT DELETING LEADING '0'.
*     SHIFT  WA_VBRK-KUNRG LEFT DELETING LEADING '0'.
*     SHIFT  WA_VBRK-KUNAG  LEFT DELETING LEADING '0'.
*     SHIFT  WA_VBRK-KUNAG  LEFT DELETING LEADING '0'.
...
*    SELECT                           "HIDDED BY SAVARIAR S AS ON 16/06/2015
*        MATNR
*        WERKS
*        FROM MARC INTO  TABLE IT_MARC FOR ALL ENTRIES IN IT_VBRK WHERE MATNR = IT_VBRK-MATNR AND WERKS = IT_VBRK-VKBUR .
...
*
*  IF IT_MARC[] IS NOT INITIAL.  HIDDED BY SAVARIAR AS ON 16/06/2015
*
*    SELECT
*         MATNR
*         MVGR1
...
*   ENDIF.
  ...
  IF IT_MARA[] IS NOT INITIAL.
    ...
    SELECT
         MATNR
         MVGR1
     FROM MVKE INTO TABLE IT_MVKE FOR ALL ENTRIES IN IT_MARA WHERE MATNR = IT_MARA-MATNR AND MVGR1 IN SO_MVGR1.
  ...
  IF IT_MARA[] IS NOT INITIAL.
    ...
    SELECT
       MATNR
       MAKTX  FROM MAKT INTO TABLE GT_MAKT FOR ALL ENTRIES IN IT_MARA WHERE MATNR = IT_MARA-MATNR .

  ENDIF.

  "***************************Sales Pro Customers**Ending*********************************************
...
*      LOOP AT IT_MARA INTO WA_MARA WHERE MATNR = WA_VBRK-MATNR.
      ...
      READ TABLE IT_MARA INTO WA_MARA WITH KEY MATNR = WA_VBRK-MATNR.
...
*  CLEAR  WA_FIELDCAT.
  ...
  WA_FIELDCAT-SELTEXT_M   = 'April Qty(Case)'.
...
*  WA_FIELDCAT-CFIELDNAME   = 'KUNAG'.
  ...
  WA_FIELDCAT-SELTEXT_M   = 'June Qty(Case)'.
