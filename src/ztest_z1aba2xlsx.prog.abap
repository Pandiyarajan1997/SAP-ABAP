...
INCLUDE ZEXCEL_FORMAT_MULTIPLE."zdemo_excel_outputopt_incl.
...
START-OF-SELECTION.
  ...
  lo_worksheet->set_cell( ip_column = 'A' ip_row = 1 ip_value = 'This is the first sheet' ip_style = lo_style_guid    ).
...
* Set color to tab with sheetname   - Green
  ...
  lo_worksheet->set_tabcolor( lv_tabcolor ).
  lo_worksheet->zif_excel_sheet_properties~selected = zif_excel_sheet_properties=>c_selected.
  lo_worksheet->set_cell( ip_column = 'A' ip_row = 1 ip_value = 'This is the second sheet' ).
    lo_worksheet->set_cell( ip_column = 'B' ip_row = 1 ip_value = 'This is the second sheet' ).
