REPORT ZKE2N_BDC1
       NO STANDARD PAGE HEADING LINE-SIZE 255.


TYPE-POOLS TRUXS.

TYPE-POOLS SLIS.

TYPES:BEGIN OF STR,

    VRGAR TYPE CEST1-VRGAR,
    BUDAT TYPE CHAR20,
    BELNR TYPE CEST0-BELNR,
    RBELN TYPE CEST0-RBELN,
    RPOSN TYPE CEST0-RPOSN,

    GT1 TYPE CHAR10,
      GT TYPE CHAR10,
  END OF STR.






TYPES:BEGIN OF STR1,
      STATUS(10) TYPE C,
      MESSAGE(50) TYPE C,
      FIELD(132) TYPE C,
  END OF STR1.


DATA:WA_STR1 TYPE STR1,
      IT_STR1 TYPE TABLE OF STR1.


DATA:WA_FCAT TYPE SLIS_FIELDCAT_ALV,
      IT_FCAT TYPE SLIS_T_FIELDCAT_ALV,
      WA_LAYOUT TYPE SLIS_LAYOUT_ALV.



DATA:WA TYPE STR,
      IT TYPE TABLE OF STR,
     LV_FILE TYPE RLGRAP-FILENAME.


DATA:LV_RAW TYPE TRUXS_T_TEXT_DATA.

DATA:BDCDATA LIKE BDCDATA OCCURS 0 WITH HEADER LINE.

DATA:IT_MSGTAB LIKE BDCMSGCOLL OCCURS 0 WITH HEADER LINE,
      WA_MSGTAB LIKE BDCMSGCOLL.










SELECTION-SCREEN BEGIN OF BLOCK A1 WITH FRAME TITLE TEXT-000.

PARAMETERS:P_FILE TYPE RLGRAP-FILENAME OBLIGATORY,
           P_MODE TYPE CTU_PARAMS-DISMODE DEFAULT 'A',
           P_UPDATE TYPE CTU_PARAMS-UPDMODE DEFAULT 'A' NO-DISPLAY.

SELECTION-SCREEN END OF BLOCK A1.


AT SELECTION-SCREEN ON VALUE-REQUEST FOR P_FILE.

  CALL FUNCTION 'KD_GET_FILENAME_ON_F4'
   EXPORTING
     PROGRAM_NAME        = SYST-REPID
     DYNPRO_NUMBER       = SYST-DYNNR
     FIELD_NAME          = 'P_FILE'
*   STATIC              = ' '
*   MASK                = ' '
*   FILEOPERATION       = 'R'
*   PATH                =
    CHANGING
      FILE_NAME           = P_FILE
*   LOCATION_FLAG       = 'P'
* EXCEPTIONS
*   MASK_TOO_LONG       = 1
*   OTHERS              = 2
            .
  IF SY-SUBRC <> 0.
* Implement suitable error handling here
  ENDIF.

START-OF-SELECTION.


*  WA-GT1 = '0'.


  LV_FILE = P_FILE.

  CALL FUNCTION 'TEXT_CONVERT_XLS_TO_SAP'
    EXPORTING
*   I_FIELD_SEPERATOR          =
   I_LINE_HEADER              = 'X'
      I_TAB_RAW_DATA             = LV_RAW
      I_FILENAME                 = LV_FILE
    TABLES
      I_TAB_CONVERTED_DATA       = IT
* EXCEPTIONS
*   CONVERSION_FAILED          = 1
*   OTHERS                     = 2
            .
  IF SY-SUBRC <> 0.
* Implement suitable error handling here
  ENDIF.









  LOOP AT IT INTO WA.



    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0100'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'CEST0-RPOSN'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=NEXT'.
    PERFORM BDC_FIELD       USING 'CEST1-VRGAR'
                                  WA-VRGAR."'F'.
    PERFORM BDC_FIELD       USING 'CEST1-BUDAT'
                                  WA-BUDAT."'01.12.2013'.
    PERFORM BDC_FIELD       USING 'RADIOVAL1'
                                  'X'.
    PERFORM BDC_FIELD       USING 'RADIOCURR1'
                                  'X'.
    PERFORM BDC_FIELD       USING 'CEST0-BELNR'
                                  WA-BELNR."'1662'.
    PERFORM BDC_FIELD       USING 'CEST0-RBELN'
                                  WA-RBELN."'101000006'.
    PERFORM BDC_FIELD       USING 'CEST0-RPOSN'
                                  WA-RPOSN.                 "'000007'.
    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=TAB2'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES-VALUE(02)'.
    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(03)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.



    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(03)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    0'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(02)'
*                              '                    0'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(03)'
*                              '                    0'.

    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
*                                  WA-GT1."'                   0'.
     PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.



    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(02)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.



*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
*                                  WA-GT."'                 100'.




    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  WA-GT1."'                 100'.


*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.




*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    0'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(02)'
*                              '                    0'.
    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(02)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.



    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  wa-gt."'                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.



*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(02)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    0'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(02)'
*                              '                    0'.

    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.



    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(01)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              WA-GT."'                 100'.


    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.



    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(01)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    0'.

    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.


    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(01)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    0'.


    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.


    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(01)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    0'.


    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.


    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=P+'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(02)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                   0'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(02)'
*                              '                   0'.


    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.


    PERFORM BDC_DYNPRO      USING 'SAPMKEI2' '0200'.
    PERFORM BDC_FIELD       USING 'BDC_OKCODE'
                                  '=BUCH'.
    PERFORM BDC_FIELD       USING 'BDC_CURSOR'
                                  'GT_LINES_VAL-VALUE(01)'.
    PERFORM BDC_FIELD       USING 'CEST1-FRWAE'
                                  'INR'.
*perform bdc_field       using 'GT_LINES_VAL-VALUE(01)'
*                              '                    1'.



    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(01)'
                                  '                  0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(02)'
                                  '                   0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(03)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(04)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(05)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(06)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(07)'
                                  '                    0'.
    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(08)'
                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(09)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(10)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(11)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(12)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(13)'
*                                  '                    0'.
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(14)'
*                                  '                    0'.
*
*
*    PERFORM BDC_FIELD       USING 'GT_LINES_VAL-VALUE(15)'
*                                  '                    0'.




*perform bdc_transaction using 'KE21N'.



    CALL TRANSACTION 'KE21N' USING BDCDATA MODE P_MODE UPDATE P_UPDATE MESSAGES INTO IT_MSGTAB.
    CLEAR BDCDATA.
    REFRESH  BDCDATA[].

    READ TABLE IT_MSGTAB INTO WA_MSGTAB WITH KEY MSGTYP = 'E'.

    IF SY-SUBRC = 0.

      WA_STR1-STATUS = 'Error'.
      WA_STR1-FIELD  = WA_MSGTAB-FLDNAME.
      CONCATENATE WA-BELNR 'Ended with Error' INTO WA_STR1-MESSAGE SEPARATED BY SPACE.

    ELSE.


      WA_STR1-STATUS = 'Sucess'.

      CONCATENATE WA-BELNR 'Saved Sucessfully' INTO WA_STR1-MESSAGE SEPARATED BY SPACE.

    ENDIF.

    APPEND WA_STR1 TO IT_STR1.

    CLEAR:WA_STR1,WA_MSGTAB.

    REFRESH IT_MSGTAB[].

  ENDLOOP.



  CLEAR : WA_FCAT .
  WA_FCAT-FIELDNAME = 'STATUS' .
  WA_FCAT-COL_POS = 1 .
  WA_FCAT-SELTEXT_L = 'Status' .
  WA_FCAT-OUTPUTLEN = 10 .
*  WA_FCAT-TABNAME = 'IT_FINAL'.
  APPEND WA_FCAT TO IT_FCAT.



  CLEAR : WA_FCAT .
  WA_FCAT-FIELDNAME = 'MESSAGE' .
  WA_FCAT-COL_POS = 2 .
  WA_FCAT-SELTEXT_L = 'Message' .
  WA_FCAT-OUTPUTLEN = 50 .
*  WA_FCAT-TABNAME = 'IT_FINAL'.
  APPEND WA_FCAT TO IT_FCAT.

*
*  CLEAR : WA_FCAT .
*  WA_FCAT-FIELDNAME = 'FIELD' .
*  WA_FCAT-COL_POS = 3 .
*  WA_FCAT-SELTEXT_L = 'Field Name' .
*  WA_FCAT-OUTPUTLEN = 50 .
**  WA_FCAT-TABNAME = 'IT_FINAL'.
*  APPEND WA_FCAT TO IT_FCAT.


  WA_LAYOUT-COLWIDTH_OPTIMIZE = 'X'.



  CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
   EXPORTING
*   I_INTERFACE_CHECK                 = ' '
*   I_BYPASSING_BUFFER                = ' '
*   I_BUFFER_ACTIVE                   = ' '
*   I_CALLBACK_PROGRAM                = ' '
*   I_CALLBACK_PF_STATUS_SET          = ' '
*   I_CALLBACK_USER_COMMAND           = ' '
*   I_CALLBACK_TOP_OF_PAGE            = ' '
*   I_CALLBACK_HTML_TOP_OF_PAGE       = ' '
*   I_CALLBACK_HTML_END_OF_LIST       = ' '
*   I_STRUCTURE_NAME                  =
*   I_BACKGROUND_ID                   = ' '
*   I_GRID_TITLE                      =
*   I_GRID_SETTINGS                   =
     IS_LAYOUT                         = WA_LAYOUT
     IT_FIELDCAT                       = IT_FCAT
*   IT_EXCLUDING                      =
*   IT_SPECIAL_GROUPS                 =
*   IT_SORT                           =
*   IT_FILTER                         =
*   IS_SEL_HIDE                       =
*   I_DEFAULT                         = 'X'
*   I_SAVE                            = ' '
*   IS_VARIANT                        =
*   IT_EVENTS                         =
*   IT_EVENT_EXIT                     =
*   IS_PRINT                          =
*   IS_REPREP_ID                      =
*   I_SCREEN_START_COLUMN             = 0
*   I_SCREEN_START_LINE               = 0
*   I_SCREEN_END_COLUMN               = 0
*   I_SCREEN_END_LINE                 = 0
*   I_HTML_HEIGHT_TOP                 = 0
*   I_HTML_HEIGHT_END                 = 0
*   IT_ALV_GRAPHICS                   =
*   IT_HYPERLINK                      =
*   IT_ADD_FIELDCAT                   =
*   IT_EXCEPT_QINFO                   =
*   IR_SALV_FULLSCREEN_ADAPTER        =
* IMPORTING
*   E_EXIT_CAUSED_BY_CALLER           =
*   ES_EXIT_CAUSED_BY_USER            =
    TABLES
      T_OUTTAB                          = IT_STR1
* EXCEPTIONS
*   PROGRAM_ERROR                     = 1
*   OTHERS                            = 2
            .
  IF SY-SUBRC <> 0.
* Implement suitable error handling here
  ENDIF.








*----------------------------------------------------------------------*
*        Start new screen                                              *
*----------------------------------------------------------------------*
FORM BDC_DYNPRO USING PROGRAM DYNPRO.
  CLEAR BDCDATA.
  BDCDATA-PROGRAM  = PROGRAM.
  BDCDATA-DYNPRO   = DYNPRO.
  BDCDATA-DYNBEGIN = 'X'.
  APPEND BDCDATA.
ENDFORM.                    "BDC_DYNPRO

*----------------------------------------------------------------------*
*        Insert field                                                  *
*----------------------------------------------------------------------*
FORM BDC_FIELD USING FNAM FVAL.
*  IF FVAL <> NODATA.
  CLEAR BDCDATA.
  BDCDATA-FNAM = FNAM.
  BDCDATA-FVAL = FVAL.
  APPEND BDCDATA.
*  ENDIF.
ENDFORM.                    "BDC_FIELD
