...
*               bukrs IN ('1000').
  ...
  IF l_tab_vbrk[] IS NOT INITIAL.
    ...
    SELECT knumv "#EC CI_DB_OPERATION_OK[2220005] " Added by <IT-CAR Tool> during Code Remediation
           kposn
           kschl
           kbetr
           kwert
    ...
    IF sy-tabix = 1.
      ...
      IF sy-subrc = 0.
        ...
        CASE w_tab_vbrk-fkart.
          WHEN 'YRF2' OR 'YBFS' OR 'YBDP' OR 'YBBR' OR 'IV' OR 'YSTO' OR 'YBTE' OR 'YBEO'.  "
            p_e_in-doctyp    = 'INV'.
...
*Item Is Servcice
    ...
    IF w_tab_marc-steuc IS NOT INITIAL.
      ...
      wa_itemdtls-steuc = w_tab_marc-steuc.
    ELSE.
...
**          lv_igstrate           = w_tab_prcd1-kbetr / 10. "wa_itemdtls-igstamt.
          lv_igstrate           = w_tab_prcd1-kbetr. "wa_itemdtls-igstamt.
        ELSE.
...
*Added On: 27.09.2023
  ...
  IF gt_response IS NOT INITIAL.
    ...
    READ TABLE gt_response INTO DATA(gs_response) INDEX 1.
...
*  SHIFT p_einv_out-vbeln LEFT DELETING LEADING '0'.
  ...
  IF gs_response-signedinvoice IS NOT INITIAL.

    wa_j_1ig_invrefnum-signed_inv    = gs_response-signedinvoice.
    wa_j_1ig_invrefnum-signed_qrcode = gs_response-signedqrcode.
    CLEAR gs_response.
...
*& <--  p2        text
*&---------------------------------------------------------------------*
FORM response_deserialize USING response.
