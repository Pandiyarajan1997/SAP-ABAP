...
*        SELECT  SINGLE * INTO WA_KONV FROM PRCD_ELEMENTS
       "Added by SPLABAP during code remediation
        SELECT  * INTO WA_KONV FROM PRCD_ELEMENTS UP TO 1 ROWS
          WHERE KNUMV = WA_EKKO-KNUMV AND KPOSN = IT_EKPO-EBELP AND KSCHL = 'P101'  " KONV Replaced With PRCD_ELEMENTS Added by <IT-CAR Tool> during Code Remediation - tool
       ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
...
*         G_EXCISE2 =  IT_EKPO-NETWR / 100 * DKWERT.
      ...
      CLEAR : WA_A363, GKWERT, DKWERT, BEXDUT.
      "Commented by SPLABAP during code remediation
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR AND KSCHL = 'JSEP' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
     "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS
        AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR AND KSCHL = 'JSEP' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
        "Added by SPLABAP during code remediation
...
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR AND KSCHL = 'JMIP'  AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
     "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A363 FROM A363 UP TO 1 ROWS
         ...
         AND LIFNR = L_DOC-XEKKO-LIFNR AND KSCHL = 'JMIP'  AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
        "Commented by SPLABAP during code remediation
...
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A363-KNUMH ."AND LOEVM_KO <> 'X' .
       "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A363-KNUMH "AND LOEVM_KO <> 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          GKWERT =  WA_KONP-KBETR .
...
*         G_EXCISE5 =  IT_EKPO-NETWR / 100 * DKWERT.
      ...
      CLEAR WA_KONP.
      CLEAR : WA_A363, GKWERT, DKWERT, BEXDUT.
      "ACommeted by SPLABAP during code remediation
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR AND KSCHL = 'JESI' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
      "Added by SPLABAP during code remediation
      SELECT * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR
        AND LIFNR = L_DOC-XEKKO-LIFNR AND KSCHL = 'JESI' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
        "Commented by SPLABAP during code remediation
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A363-KNUMH. "AND LOEVM_KO <> 'X'.
      "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP FROM KONP UP TO 1 ROWS WHERE KNUMH =  WA_A363-KNUMH "AND LOEVM_KO <> 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          GKWERT =  WA_KONP-KBETR .
...
*      SELECT SINGLE * FROM EKKO INTO WA_EKKO WHERE EBELN = IT_EKPO-EBELN.
      "Added by SPLABAP during code remediation
      SELECT * FROM EKKO INTO WA_EKKO  UP TO 1 ROWS
...
*        SELECT SINGLE * FROM EKBE INTO WA_EKBE WHERE EBELN = WA_EKKO-EBELN AND EBELP = IT_EKPO-EBELP AND BWART = '351'.
        ...
        SELECT  * FROM EKBE INTO WA_EKBE UP TO 1 ROWS
           WHERE EBELN = WA_EKKO-EBELN AND EBELP = IT_EKPO-EBELP AND BWART = '351'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          SELECT  SINGLE * INTO WA_KONV FROM PRCD_ELEMENTS WHERE KNUMV = WA_EKKO-KNUMV AND KPOSN = IT_EKPO-EBELP AND KSCHL = 'P101' . " KONV Replaced With PRCD_ELEMENTS Added by <IT-CAR Tool> during Code Remediation - tool
...
*              SELECT SINGLE * FROM J_1IEXCDTL INTO WA_EXCDTL WHERE EXNUM = WA_RG23D-EXNUM AND ZEILE = WA_RG23D-ZEILE.
             "Added by SPLABAP during code remediation
              SELECT * FROM J_1IEXCDTL INTO WA_EXCDTL UP TO 1 ROWS WHERE EXNUM = WA_RG23D-EXNUM AND ZEILE = WA_RG23D-ZEILE
                ORDER BY PRIMARY KEY.
                ENDSELECT.
...
*        SELECT  SINGLE * INTO WA_KONP  FROM KONP WHERE KNUMH =  WA_A359-KNUMH ."AND LOEVM_KO <> 'X'.
        "Added by SPLABAP during code remediation
        SELECT  * INTO WA_KONP
           UP TO 1 ROWS  FROM KONP WHERE KNUMH =  WA_A359-KNUMH "AND LOEVM_KO <> 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
...
*      SELECT SINGLE * INTO WA_A359 FROM A359 WHERE WERKS = IT_EKPO-WERKS  AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JEC3' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
     "Added by SPLABAP during code remediation
      SELECT * INTO WA_A359 FROM A359 UP TO 1 ROWS
        WHERE WERKS = IT_EKPO-WERKS  AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JEC3' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
        "Comented by SPLABAP during code remediation
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A359-KNUMH ."AND LOEVM_KO <> 'X'.
        "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP FROM KONP UP TO 1 ROWS
...
*      SELECT SINGLE * INTO WA_A359 FROM A359 WHERE WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSE1' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
      "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A359 FROM A359 UP TO 1 ROWS
         WHERE WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSE1' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A359-KNUMH. " AND LOEVM_KO <> 'X'.
       "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP UP TO 1 ROWS
          FROM KONP WHERE KNUMH =  WA_A359-KNUMH " AND LOEVM_KO <> 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          GKWERT = GKWERT + WA_KONP-KBETR.
      ...
      CLEAR WA_A359.
      "Commented by SPLABAP during code remediation
*      SELECT SINGLE * INTO WA_A359 FROM A359
      "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A359 FROM A359 UP TO 1 ROWS
         WHERE WERKS = IT_EKPO-WERKS  AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSV2' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
        "Commented by SPLABAP during code remediation
...
*      SELECT SINGLE * INTO WA_A359 FROM A359 WHERE WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JEC4' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
      "Added by SPLABAP during code remediation
      SELECT * INTO WA_A359 FROM A359 UP TO 1 ROWS
        WHERE WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JEC4' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
        "Commented by SPLABAP during code remediation
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A359-KNUMH ."AND LOEVM_KO <> 'X'.
        "Added by SPLABAP during code remediation
        SELECT  * INTO WA_KONP FROM KONP UP TO 1 ROWS WHERE KNUMH =  WA_A359-KNUMH "AND LOEVM_KO <> 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          GKWERT = GKWERT + WA_KONP-KBETR.
...
*      SELECT SINGLE * INTO WA_A359 FROM A359 WHERE WERKS = IT_EKPO-WERKS  AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSE2' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
      "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A359 FROM A359 UP TO 1 ROWS
        WHERE WERKS = IT_EKPO-WERKS  AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSE2' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
      IF SY-SUBRC = 0.
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A359-KNUMH ."AND LOEVM_KO <> 'X'.
...
*      SELECT SINGLE * INTO WA_A359 FROM A359  WHERE WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSVD' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
     "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A359 FROM A359 UP TO 1 ROWS
          WHERE WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND KSCHL = 'JSB5' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
...
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A359-KNUMH . " OR KNUMH = WA_A969-KNUMH ."AND LOEVM_KO <> 'X'.
       "Added by SPLABAP during code remediation
        SELECT  * INTO WA_KONP FROM KONP UP TO 1 ROWS WHERE KNUMH =  WA_A359-KNUMH
          ORDER BY PRIMARY KEY . " OR KNUMH = WA_A969-KNUMH ."AND LOEVM_KO <> 'X'.
          ENDSELECT.
        IF SY-SUBRC = 0.
          GKWERT = GKWERT + WA_KONP-KBETR.
        ENDIF.
...
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVRD' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
     "Added by SPLABAP during code remediation
      SELECT * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS
        AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVRD' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
...
*    SELECT SINGLE * INTO WA_A359 FROM A359 WHERE KSCHL = 'JVRD' AND WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND DATBI GE LV_DATE AND DATAB LE LV_DATE .
    "Added by SPLABAP during code remediation
    SELECT * INTO WA_A359 FROM A359 UP TO 1 ROWS WHERE KSCHL = 'JVRD'
       AND WERKS = IT_EKPO-WERKS AND MATKL = IT_EKPO-MATKL AND DATBI GE LV_DATE AND DATAB LE LV_DATE
      ORDER BY PRIMARY KEY .
      ENDSELECT.
 ENDIF.
...
"Added by SPLABAP during code remediation
   SELECT  * INTO WA_A998 UP TO 1 ROWS FROM A998 WHERE KSCHL ='JVRD'
       AND WERKS = IT_EKPO-WERKS AND LIFNR = L_DOC-XEKKO-LIFNR AND MATKL = IT_EKPO-MATKL
     ORDER BY PRIMARY KEY .
     ENDSELECT.
...
"Commetedd by SPLABAP during code remediation
*   SELECT SINGLE * INTO WA1_A998 FROM A998 WHERE KSCHL ='JVCS'  AND WERKS = IT_EKPO-WERKS AND LIFNR = L_DOC-XEKKO-LIFNR AND MATKL = IT_EKPO-MATKL .
   ...
   SELECT  * INTO WA1_A998 FROM A998 UP TO 1 ROWS
      WHERE KSCHL ='JVCS'  AND WERKS = IT_EKPO-WERKS AND LIFNR = L_DOC-XEKKO-LIFNR AND MATKL = IT_EKPO-MATKL
     ORDER BY PRIMARY KEY.
     ENDSELECT.
...
"ADDED ON 16/2
  ...
  "Added by SPLABAP during code remediation
  SELECT   * INTO WA_KONP FROM KONP  UP TO 1 ROWS
    WHERE KNUMH =  WA_A363-KNUMH OR KNUMH = WA_A969-KNUMH OR KNUMH = WA_A998-KNUMH "AND LOEVM_KO <> 'X'. " Added By Govind LOEVM_KO
    ORDER BY PRIMARY KEY.
    ENDSELECT.
...
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND  MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVRN' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
   "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS
        AND  MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVRN' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
...
*       SELECT SINGLE * INTO WA2_A998 FROM A998 WHERE KSCHL ='JVRN'  AND WERKS = IT_EKPO-WERKS AND LIFNR = L_DOC-XEKKO-LIFNR AND MATKL = IT_EKPO-MATKL .
       "Added by SPLABAP during code remediation
       SELECT  * INTO WA2_A998 FROM A998 UP TO 1 ROWS
         WHERE KSCHL ='JVRN'  AND WERKS = IT_EKPO-WERKS AND LIFNR = L_DOC-XEKKO-LIFNR AND MATKL = IT_EKPO-MATKL
         ORDER BY PRIMARY KEY.
         ENDSELECT.
...
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A363-KNUMH  OR KNUMH = WA_A969-KNUMH OR KNUMH = WA2_A998-KNUMH AND LOEVM_KO NE 'X'.
       "Added by SPLABAP during code remediation
        SELECT  * INTO WA_KONP FROM KONP UP TO 1 ROWS
           WHERE KNUMH =  WA_A363-KNUMH  OR KNUMH = WA_A969-KNUMH OR KNUMH = WA2_A998-KNUMH AND LOEVM_KO NE 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          VKWERT =  VKWERT + WA_KONP-KBETR .
...
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVCD' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
      "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS
        AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVCD' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
...
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVCN' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
     "Added by SPLABAP during code remediation
      SELECT * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS
        AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVCN' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
 IF WA_A363 IS INITIAL .
   "Commented by SPLABAP during code remediation
*    SELECT SINGLE * INTO WA_A969 FROM A969 WHERE KSCHL = 'JVCN' AND MWSKZ = IT_EKPO-MWSKZ  . " AND DATBI GE LV_DATE AND DATAB LE LV_DATE..
  "Added by SPLABAP during code remediation
    SELECT * INTO WA_A969 FROM A969 UP TO 1   ROWS WHERE KSCHL = 'JVCN' AND MWSKZ = IT_EKPO-MWSKZ
...
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE ( KNUMH =  WA_A363-KNUMH OR KNUMH = WA_A969-KNUMH ) AND LOEVM_KO NE 'X'.
       "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP UP TO 1 ROWS
           FROM KONP WHERE ( KNUMH =  WA_A363-KNUMH OR KNUMH = WA_A969-KNUMH )
          AND LOEVM_KO NE 'X'
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
...
*      SELECT SINGLE * INTO WA_A363 FROM A363 WHERE WERKS = IT_EKPO-WERKS AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVCS' AND DATBI GE LV_DATE AND DATAB LE LV_DATE.
      "Added by SPLABAP during code remediation
      SELECT  * INTO WA_A363 FROM A363 UP TO 1 ROWS WHERE WERKS = IT_EKPO-WERKS
        AND MATNR = IT_EKPO-MATNR AND LIFNR = L_DOC-XEKKO-LIFNR  AND KSCHL = 'JVCS' AND DATBI GE LV_DATE AND DATAB LE LV_DATE
        ORDER BY PRIMARY KEY.
        ENDSELECT.
  IF WA_A363 IS INITIAL .
    "Commented by SPLABAP during code remediation
*    SELECT SINGLE * INTO WA_A969 FROM A969 WHERE KSCHL = 'JVCS' AND MWSKZ = IT_EKPO-MWSKZ . " AND DATBI GE LV_DATE AND DATAB LE LV_DATE..
   "Added by SPLABAP during code remediation
    SELECT * INTO WA_A969 FROM A969 UP TO 1 ROWS
...
*        SELECT SINGLE * INTO WA_A003 FROM A003 WHERE KSCHL = 'JNLD' AND MWSKZ = IT_EKPO-MWSKZ .
       "Added by SPLABAP during code remediation
        SELECT  * INTO WA_A003 UP TO 1 ROWS FROM A003 WHERE KSCHL = 'JNLD' AND MWSKZ = IT_EKPO-MWSKZ
          ORDER BY PRIMARY KEY.
          ENDSELECT.
...
*                 GKWERT = GKWERT + WA_KONP-KBETR.
*                  DKWERT =  GKWERT / 10.
*                 B_DUTY12 =   WA_PO-CTOT / 100  * DKWERT.
                    VKWERT =  VKWERT + WA_KONP-KBETR .
                    WA_PO-KSCHL =  WA_KONP-KSCHL.
