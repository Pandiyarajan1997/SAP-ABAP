...
TYPES:BEGIN OF TY_OUTPUT,
  ...
  GOR_QUN TYPE MSEG-MENGE,      "GOODS RECEIPT QUANTITY
  ...
  POVSGI TYPE CHAR3,       "DAYS OF GOODSISSUE
  POVSGR TYPE CHAR3,       "DAYS OF GOODSRECEIPT
  END OF TY_OUTPUT.
...
*       E_YEARS        =
    ...
    IF MON <= 12.
      ...
      SELECT EBELN RESWK BEDAT FROM EKKO INTO TABLE IT_EKKO WHERE RESWK IN S_RESWK AND BEDAT IN S_BEDAT AND BSART = 'UB'.

      IF IT_EKKO IS NOT INITIAL.

        SELECT EBELN EBELP MATNR WERKS MENGE MEINS NETWR FROM EKPO INTO TABLE IT_EKPO FOR ALL ENTRIES IN IT_EKKO
                                                                                WHERE EBELN EQ IT_EKKO-EBELN AND WERKS IN S_WERKS.
        IF IT_EKPO IS NOT INITIAL.
          ...
          SELECT EBELN EBELP BELNR BUZEI BWART DMBTR FROM EKBE INTO TABLE IT_EKBE FOR ALL ENTRIES IN IT_EKPO WHERE EBELN EQ IT_EKPO-EBELN AND EBELP EQ IT_EKPO-EBELP
                                                                                                        AND ( BWART = '351' OR BWART = '101' ).

          SELECT WERKS NAME1 REGIO FROM T001W INTO TABLE IT_T001W FOR ALL ENTRIES IN IT_EKPO WHERE WERKS EQ IT_EKPO-WERKS.

          SELECT SPRAS LAND1 BLAND BEZEI FROM T005U INTO TABLE IT_T005U FOR ALL ENTRIES IN IT_T001W WHERE BLAND EQ IT_T001W-REGIO
                                                                                                 AND SPRAS EQ 'EN' AND LAND1 EQ 'IN'.

          SELECT MBLNR MJAHR ZEILE BWART MATNR CHARG DMBTR MENGE EBELN EBELP BUDAT_MKPF FROM MSEG INTO TABLE IT_MSEG
                 FOR ALL ENTRIES IN IT_EKPO WHERE EBELN EQ IT_EKPO-EBELN AND EBELP EQ IT_EKPO-EBELP AND XAUTO <> 'X' AND ( BWART = '351' OR BWART = '101' ).

          SELECT MBLNR MJAHR ZEILE BWART MATNR CHARG DMBTR MENGE EBELN EBELP SMBLN SMBLP BUDAT_MKPF FROM MSEG INTO TABLE T352
                 FOR ALL ENTRIES IN IT_EKPO WHERE EBELN EQ IT_EKPO-EBELN AND EBELP EQ IT_EKPO-EBELP AND XAUTO <> 'X' AND ( BWART = '352' OR BWART = '102' ).
        ELSE.
...
FORM READ_DATA .
  ...
  LOOP AT T352 INTO W352 .
    DELETE IT_MSEG WHERE MBLNR = W352-SMBLN AND ZEILE = W352-SMBLP AND MENGE = W352-MENGE .
  ENDLOOP.
  ...
  IF IT_EKPO IS NOT INITIAL.
    LOOP AT IT_EKPO INTO WA_EKPO.
      ...
      LOOP AT IT_MSEG INTO WA_MSEG WHERE EBELN = WA_EKPO-EBELN AND EBELP = WA_EKPO-EBELP AND BWART = '351'.
*        READ TABLE T352 INTO W352 WITH KEY EBELN = WA_MSEG-EBELN EBELP = WA_MSEG-EBELP MATNR = WA_MSEG-MATNR MENGE = WA_MSEG-MENGE
*                                                                                       DMBTR = WA_MSEG-DMBTR CHARG = WA_MSEG-CHARG BWART = 352.
*        IF W352 IS INITIAL.
        READ TABLE IT_MSEG1 INTO WA_MSEG1 WITH KEY EBELN = WA_MSEG-EBELN EBELP = WA_MSEG-EBELP BWART = '101' MENGE = WA_MSEG-MENGE .
        CLEAR : WA_EKBE.
...
FORM DISPLAY .
  ...
  WA_HEAD-TYP = 'H'.
  WA_HEAD-INFO = 'Stock Transfer Order Details'.
