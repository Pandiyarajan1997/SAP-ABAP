"Name: \PR:RQCAAP02\FO:INITIALIZE_AFTER_PROCESSING\SE:BEGIN\EI
...
if P_PARNR is NOT INITIAL.

  SELECT SINGLE * FROM lfa1 INTO ls_lfa1 WHERE lifnr = P_PARNR.
  SELECT SINGLE  * FROM T001K INTO LV_T001K WHERE BWKEY = LS_lfa1-WERKS.
*   SELECT SINGLE  * from qals INTO ls_qals WHERE matnr = P_MATNR and
*                                      charg = p_CHARG .            "Commented by SPLABAP during code remediation
   SELECT * from qals UP TO 1 ROWS INTO ls_qals WHERE matnr = P_MATNR and
                                      charg = p_CHARG ORDER BY PRIMARY KEY. "Added by SPLABAP during code remediation
     ENDSELECT.
else.
...
*          SELECT SINGLE  * from qals INTO ls_qals WHERE matnr = P_MATNR and    "Commented by SPLABAP during code remediation
          SELECT * from qals UP TO 1 ROWS INTO ls_qals WHERE matnr = P_MATNR and  "Added by SPLABAP during code remediation
                                       charg = p_CHARG ORDER BY PRIMARY KEY.  "Added by SPLABAP during code remediation
...
*           SELECT SINGLE * from qals INTO ls_qals WHERE matnr = VBDPL-MATNR and
*                                      charg = vbdpl-CHARG and
*                                      WERK  = vbdpl-WERKS.                 "Commented by SPLABAP during code remediation
           SELECT * from qals UP TO 1 ROWS INTO ls_qals WHERE matnr = VBDPL-MATNR and
                                      charg = vbdpl-CHARG and
                                      WERK  = vbdpl-WERKS ORDER BY PRIMARY KEY. "Added by SPLABAP during code remediation
             ENDSELECT.
...
* Implement suitable error handling here
  ...
  ELSEIF LV_T001K-BUKRS = '1400'." OR VBDPL-WERKS = '1005'.
