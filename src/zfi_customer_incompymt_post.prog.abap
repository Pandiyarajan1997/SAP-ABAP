...
DATA: lv_kunnr TYPE zfi_cust_inpymt-kunnr,
      lv_xblnr TYPE zfi_cust_inpymt-xblnr.
...
START-OF-SELECTION.
  ...
  IF sy-subrc <> 0.
    ...
    WRITE : / 'Error:',l_msg.
  ELSE.
**** Company code getting from input ***
*    SELECT SINGLE * INTO @DATA(ls_tvarvc1) FROM tvarvc
*                      WHERE name = 'CUSTOMER_INPYMT_CC'
*                      AND type = 'P'.
*    CLEAR lv_bukrs.
*    lv_bukrs = ls_tvarvc1-low.
*    IF sy-subrc EQ 0.
***** Currency Key for Company code ***
*      SELECT SINGLE waers FROM t001 INTO @DATA(lv_waers)
...
**** Selecting the payments which or not processed from Log table ***
    SELECT * FROM zfi_cust_inpymt
             INTO TABLE @DATA(lt_paymnt_post)
             WHERE "bukrs EQ @lv_bukrs
                 kunnr IN @s_kunnr
             AND xblnr IN @s_xblnr
             AND status EQ '10'.
    IF sy-subrc EQ 0.
      SORT lt_paymnt_post[] BY bukrs kunnr.
** Processing One by one Entries through Incoming paymen program ***
      LOOP AT lt_paymnt_post ASSIGNING FIELD-SYMBOL(<lw_paymnt_post>).
        "Check for Another Table entries whether document saved or not
        SELECT SINGLE * FROM zdebit_ref_mis INTO @DATA(l_alternate_chk) WHERE bukrs = @<lw_paymnt_post>-bukrs
                                                                        AND account_type = 'D'
                                                                        AND account = @<lw_paymnt_post>-kunnr
                                                                        AND invoice_refno = @<lw_paymnt_post>-xblnr
                                                                        AND reference_id = @<lw_paymnt_post>-reference_id.
        IF sy-subrc NE 0.
**** Currency Key for Company code ***
          SELECT SINGLE waers FROM t001 INTO @DATA(lv_waers)
                              WHERE bukrs = @<lw_paymnt_post>-bukrs.
*** function module to get fiscal year ***
          CLEAR: lv_fisyear,lv_month,l_return.
          CALL FUNCTION 'BAPI_COMPANYCODE_GET_PERIOD'
            EXPORTING
              companycodeid = <lw_paymnt_post>-bukrs
              posting_date  = sy-datum
            IMPORTING
              fiscal_year   = lv_fisyear
              fiscal_period = lv_month
          ...
          IF <lw_paymnt_post>-bukrs = '1000'.
*** g/l for Customer payment posting ****
            SELECT SINGLE * FROM tvarvc INTO @DATA(ls_tvarvc)
                              WHERE name = 'CUSTOMER_INPYMT_GL'
                              AND type = 'P'.
            <lw_paymnt_post>-gl_acnt = ls_tvarvc-low.
            CLEAR : ls_tvarvc.
          ENDIF.
          ...
          lt_seltab = VALUE #(
                           ...
                           ( selname = 'NEWKO' kind = 'P' sign = 'I' option = 'EQ' low = <lw_paymnt_post>-gl_acnt )
                           ...
                           ( selname = 'WAERS' kind = 'P' sign = 'I' option = 'EQ' low = lv_waers )
...
** Actual Program doing the posting Entries in FB05 ***
          SUBMIT zfi_incoming_payment_new WITH SELECTION-TABLE lt_seltab
                                          AND RETURN.
        ELSE.
          <lw_paymnt_post>-doc_no = l_alternate_chk-doc_no.
          <lw_paymnt_post>-status = '40'.
          <lw_paymnt_post>-remarks = |Payment Document { l_alternate_chk-doc_no } Posted Successfully|.
          MODIFY zfi_cust_inpymt FROM <lw_paymnt_post>.
        ENDIF.
        CLEAR : lv_waers.
      ENDLOOP.
