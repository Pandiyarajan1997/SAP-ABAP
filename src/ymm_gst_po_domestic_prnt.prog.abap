*&---------------------------------------------------------------------*
*& Subroutine Pool   ZMM_GST_PO_DOMESTIC_PRNT
*&
*&---------------------------------------------------------------------*
*&
*&
...
PROGRAM YMM_GST_PO_DOMESTIC_PRNT.
...
****************************************************ADDed by Bharath 09/June/2017 FOR ***CGST*****************************************************************************
***************************************************************************************************************************************************************************
      CLEAR WA_KONP.
      CLEAR : WA_A363, GKWERT, DKWERT, BEXDUT, G_EXCISE1.
      clear : wa_a003.

*      SELECT SINGLE * INTO WA_A003 FROM A003 WHERE kschl = 'JICG' AND MWSKZ = it_ekpo-MWSKZ.
*      " Added By Govind
*      IF SY-SUBRC = 0.
...
wa_ekpo1 type ekpo.
    ...
    it_taxcom-mwskz = it_ekpo-mwskz..""'Z2'."'Z1'.""ekpo-mwskz. Z1
     ...
     it_taxcom-wrbtr = ( it_EKPO-NETPR / it_ekpo-PEINH ) * it_EKPO-MENGE."#EC CI_FLDEXT_OK[2610650]
     "Added by SPLABAP during code remediation
...
*BREAK-POINT.
      ...
      IF SY-SUBRC = 0."Added by SPLABAP during code remediation
...
*            SELECT SINGLE * FROM J_1IRG23D INTO WA_RG23D WHERE MBLNR = WA_EKBE-BELNR AND WERKS = WA_EKBE-WERKS .
           "Commented by SPLABAP during code remediation
            ...
            IF SY-SUBRC = 0.
              "Added by SPLABAP during code remediation
*              SELECT SINGLE * FROM J_1IEXCDTL INTO WA_EXCDTL
              "Commented by SPLABAP during code remediation
...
******************************************************************************************************************************************
******************************************ADDed by Bharath 09/June/2017 FOR ***SGST*******************************************************
  CLEAR WA_KONP.
CLEAR : WA_KONP,GKWERT, DKWERT , B_DUTY1, B_DUTY2,  B_DUTY3,  B_DUTY4,  B_DUTY5, B_DUTY6.
      CLEAR WA_A003.
*      SELECT SINGLE * INTO WA_A003 FROM A003 WHERE kschl = 'JISG' AND MWSKZ = it_ekpo-MWSKZ.
*      IF SY-SUBRC = 0.
*        SELECT  SINGLE * INTO WA_KONP  FROM KONP WHERE KNUMH =  wa_A003-KNUMH.
*        IF SY-SUBRC = 0.
*          GKWERT = GKWERT + WA_KONP-KBETR.
...
*        SELECT  SINGLE * INTO WA_KONP  FROM KONP WHERE KNUMH =  WA_A359-KNUMH . " OR KNUMH = WA_A969-KNUMH ."AND LOEVM_KO <> 'X'.
      "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP UP TO 1 ROWS
           FROM KONP WHERE KNUMH =  WA_A359-KNUMH " OR KNUMH = WA_A969-KNUMH ."AND LOEVM_KO <> 'X'.
          ORDER BY PRIMARY KEY.
          ENDSELECT.
        IF SY-SUBRC = 0.
          GKWERT = GKWERT + WA_KONP-KBETR.
...
*        SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A359-KNUMH . " OR KNUMH = WA_A969-KNUMH ."AND LOEVM_KO <> 'X'.
        "Added by SPLABAP during code remediation
        SELECT   * INTO WA_KONP UP TO 1 ROWS FROM KONP
...
*  SELECT  SINGLE * INTO WA_KONP FROM KONP WHERE KNUMH =  WA_A363-KNUMH OR KNUMH = WA_A969-KNUMH OR KNUMH = WA_A998-KNUMH . "AND LOEVM_KO <> 'X'. " Added By Govind LOEVM_KO
...
******************************************************************************************************************************************
******************************************ADDed by Bharath 09/June/2017 FOR ***IGST*******************************************************
  CLEAR WA_KONP.
*CLEAR : WA_KONP,GKWERT, DKWERT , B_DUTY1, B_DUTY2,  B_DUTY3,  B_DUTY4,  B_DUTY5, B_DUTY6.
      CLEAR WA_A003.
*      SELECT SINGLE * INTO WA_A003 FROM A003 WHERE kschl = 'JIIG' AND MWSKZ = it_ekpo-MWSKZ.
*      IF SY-SUBRC = 0.
*        SELECT  SINGLE * INTO WA_KONP  FROM KONP WHERE KNUMH =  wa_A003-KNUMH.
*        IF SY-SUBRC = 0.
*          VKWERT =  VKWERT + WA_KONP-KBETR .
...
"Commented by SPLABAP during code remediation
*        SELECT SINGLE * INTO WA_A003 FROM A003 WHERE KSCHL = 'JVLN' AND MWSKZ = IT_EKPO-MWSKZ .
       "Added by SPLABAP during code remediation
