class ZCL_API_SCM_DELIVERY_PGI definition
  public
  final
  create public .
...
CLASS ZCL_API_SCM_DELIVERY_PGI IMPLEMENTATION.
...
** Deserialize the input our required input ***
   ...
   CHANGING
    ...
    IF gt_input IS NOT INITIAL.
      ...
      SELECT SINGLE * FROM zsd_spl_sale_hd
        INTO @DATA(l_salelog)
...
*              message  =               " Character 100
          ...
          IF sy-subrc = 0 AND gv_type = 'E'.
            lv_msg = |{ lv_msg } ; MM Posting Period Not Opened - Check with IT-SAP Team|.
          ENDIF.
...
*********************Pan No Missing for Sundaram Finance Customer******************
        SELECT SINGLE kunnr,zcustype,j_1ipanno,regio,stcd3,telf1 FROM kna1 INTO @DATA(gs_kna1) WHERE kunnr = @l_salelog-kunag.

        IF gs_kna1-zcustype IS NOT INITIAL.

          SELECT SINGLE * FROM zsd_cus_pan INTO @DATA(gs_cus_pan) WHERE cus_no = @l_salelog-kunag.
          IF gs_cus_pan-pan_no IS INITIAL.
            lv_msg = |{ lv_msg } ; Finance Customer { l_salelog-kunag } Pan No Missing - Check with SAP Team|.
          ENDIF.

          SELECT SINGLE * FROM zsd_cf_activate INTO @DATA(gs_sf_act) WHERE kunnr = @l_salelog-kunag.
          IF sy-subrc NE 0.
            lv_msg = |{ lv_msg } ; Finance Customer { l_salelog-kunag } Activation table not Maintained - Check with SAP Team|.
          ENDIF.
...
*********************Customer Code(gst) & region check*****************************
        IF gs_kna1-stcd3 IS NOT INITIAL.
          IF gs_kna1-stcd3(1) = space.
            lv_msg = |{ lv_msg } ; Customer { l_salelog-kunag } GST is incorrect - Check with Sales Co-ordinators Team|.
          ENDIF.
          ...
          IF gs_kna1-regio IS NOT INITIAL.
            ...
            IF gs_kna1-stcd3(2) <> lv_gstkey.
              lv_msg = |{ lv_msg } ; Customer { l_salelog-kunag } GST and Region not matching - check with Sales Co-ordinators Team|.
            ENDIF.
          ENDIF.
        ENDIF.
        IF gs_kna1-telf1 IS NOT INITIAL.
          IF gs_kna1-telf1 CO ' 0123456789'.
          ELSE.
            lv_msg = |{ lv_msg } ; Customer { l_salelog-kunag } Mobile no. incorrect may contain special character - Check with Sales Co-ordinators Team|.
          ENDIF.
        ENDIF.

*****************Customer PAN No checking *******************
        IF gs_kna1-j_1ipanno IS INITIAL.
          lv_msg = |{ lv_msg } ; Customer { l_salelog-kunag } PAN no is Missing - check with Sales Co-ordinators Team|.
        ENDIF.
*----------------------------------------------------------------------------------
...
*Sale Order Item Details *
        SELECT matnr, lfimg, meins FROM zsd_spl_sale_it INTO TABLE @DATA(lt_sale_itm) WHERE ordid = @gs_input-orderid.
        IF sy-subrc EQ 0.
          SORT  lt_sale_itm[] BY matnr ASCENDING.
*Getting Delivery Details *
          SELECT vbeln, posnr, matnr, werks, lfimg FROM lips
            INTO TABLE @DATA(lt_delivery)
            FOR ALL ENTRIES IN @lt_sale_itm
            WHERE matnr = @lt_sale_itm-matnr
...
*********************HSN & Profit Center Missing Check*****************************
            ...
            SELECT SINGLE matnr,steuc,prctr FROM marc INTO @DATA(gs_marc) WHERE matnr = @<fs_sale_itm>-matnr AND werks = @l_salelog-plant.
            IF sy-subrc = 0.
              IF gs_marc-steuc IS INITIAL.
                lv_msg = |{ lv_msg } ; HSN Missing for Material { <fs_sale_itm>-matnr } - Check with R & D Team|.
              ENDIF.
              IF gs_marc-prctr IS INITIAL.
                lv_msg = |{ lv_msg } ; Profit Center Missing for Material { <fs_sale_itm>-matnr } - Check with R & D Team|.
              ENDIF.
            ENDIF.
...
**********************check the netvalue*******************
              ...
              READ TABLE gt_condition TRANSPORTING cond_value INTO DATA(gv_condvalue)
              WITH KEY itm_number = gs_item-itm_number cond_type = 'PR00' BINARY SEARCH.
              IF sy-subrc <> 0 OR gv_condvalue IS INITIAL.
                lv_msg = |{ lv_msg } ; Selling Price (PR00) not Maintained for Materials { <fs_sale_itm>-matnr } - Check with Accounts Team|.
              ENDIF.

              READ TABLE gt_condition TRANSPORTING cond_value INTO gv_condvalue WITH KEY itm_number = gs_item-itm_number cond_type = 'VPRS' BINARY SEARCH.
              IF sy-subrc <> 0 OR gv_condvalue IS INITIAL.
                lv_msg = |{ lv_msg } ; Internal Price (VPRS) not Maintained for Materials { <fs_sale_itm>-matnr } - Check with Accounts Team|.
              ENDIF.

**********************Cgst & Sgst value Mismatch Check except YBTE order type*****************************
              IF l_salelog-auart NE 'YBTE'.
                ...
                IF sy-subrc NE 0.
                  ...
                  IF sy-subrc = 0.
                    ...
                    IF sy-subrc = 0.
                      IF lv_cgst <> lv_sgst.
                        lv_msg = |{ lv_msg } ; Cgst/Sgst Values are different in Sales Order for Material { <fs_sale_itm>-matnr }. Check with Accounts team and get new sales order created|.
                      ENDIF.
                    ENDIF.
                  ELSE.
                    lv_msg = |{ lv_msg } ; IGST/Cgst/Sgst not Maintained for Materials { <fs_sale_itm>-matnr } - Check with Accounts Team|.
                  ENDIF.
                ENDIF.
              ENDIF.
            ENDIF.
...
** Delivery Picklist Smartforms
                  ...
                  CALL FUNCTION 'ZSD_PICKLIST_PDF_GENERATE'
                    EXPORTING
                      im_vbeln       = lv_vbeln
                    IMPORTING
