...
*& Report Name                 : ZSD_PRODUCT_SALES_PRO                 *
...
IT_MARC1 TYPE TABLE OF TY_MARC,
...
IT_T001W TYPE TABLE OF GS_T001W.
...
SORT IT_VBRPK BY VBELN MATNR POSNR . " Added by <IT-CAR Tool> during Code Remediation
  DELETE ADJACENT DUPLICATES FROM IT_VBRPK COMPARING VBELN MATNR POSNR .
...
************************************* SALES PRO
  IF NOT IT_VBRPK[] IS INITIAL.
    SELECT MATNR WERKS LGORT FROM MARD
         INTO  TABLE IT_MARD FOR ALL ENTRIES IN IT_VBRPK WHERE MATNR = IT_VBRPK-MATNR AND WERKS = IT_VBRPK-WERKS AND LGORT = IT_VBRPK-SKU_ID.
    LOOP AT IT_MARD INTO WA_MARD.
      WA_MARC-MATNR = WA_MARD-MATNR.
      WA_MARC-WERKS  = WA_MARD-LGORT.
      APPEND WA_MARC TO IT_MARC.
      CLEAR WA_MARC.
    ENDLOOP.

      SELECT MATNR VOLUM VOLEH FROM MARA
      INTO  TABLE IT_MARA1 FOR ALL ENTRIES IN IT_VBRPK WHERE MATNR = IT_VBRPK-MATNR  .
        APPEND LINES OF IT_MARA1 TO IT_MARA.
  ENDIF.

  SELECT WERKS NAME1 FROM T001W INTO TABLE IT_T001W FOR ALL ENTRIES IN IT_VBRPK WHERE WERKS = IT_VBRPK-WERKS.
***********************************
******************* Sales Pro and Secondary Sales ******* Data Selection *********
...
*    WA_VBRK-VKBUR = WA_VBRPK-WERKS.
    READ TABLE IT_MARD INTO WA_MARD WITH KEY MATNR = WA_VBRPK-MATNR WERKS = WA_VBRPK-WERKS .
    IF SY-SUBRC = 0.
      WA_VBRK-VKBUR = WA_MARD-LGORT.
    ENDIF.
...
SORT IT_MARC BY MATNR WERKS. " Added by <IT-CAR Tool> during Code Remediation
DELETE ADJACENT DUPLICATES FROM IT_MARC COMPARING MATNR WERKS.

  LOOP AT IT_MARC  INTO WA_MARC.
    WA_FINOVR-MATNR = WA_MARC-MATNR.

    LOOP AT IT_VBRK INTO WA_VBRK WHERE  MATNR = WA_MARC-MATNR AND VKBUR = WA_MARC-WERKS .
      WA_FINOVR-VNETWR = WA_FINOVR-VNETWR + WA_VBRK-NETWR.
...
*    WA_FINOVR-FKDAT = WA_VBRK-FKDAT.
    ...
    READ TABLE GT_MAKT INTO WA_MAKT WITH  KEY MATNR = WA_MARC-MATNR.
...
*      WHEN  '1000'.
    ...
    ENDIF.
**      WHEN '6000'.
*        READ TABLE IT_T001W  INTO WA_T001W WITH KEY WERKS = WA_VBRK-VKBUR.
*        IF SY-SUBRC = 0.
*          WA_FINOVR-BEZEI = WA_T001W-NAME1.
*        ENDIF.
*    ENDCASE.
  ...
  SORT IT_FINOVR BY VNETWR DESCENDING.
  ...
  LOOP AT IT_FINOVR INTO WA_FINOVR.
    ...
    MODIFY IT_FINOVR FROM WA_FINOVR TRANSPORTING GBEZEI VOLUM.
    CLEAR WA_FINOVR.
  ENDLOOP.
  ...
  IF SO_MVGR1-LOW IS NOT INITIAL.
    DELETE IT_FINOVR WHERE MVGR1 EQ ' '.
  ENDIF.

DELETE IT_FINOVR WHERE FKIMG = 0.
...
*  CLEAR  WA_FIELDCAT.
  ...
  WA_FIELDCAT-FIELDNAME   = 'MVGR1'.           " ADDED BY RAM
  WA_FIELDCAT-SELTEXT_M   = 'MATERIAL GROUP NO'.
...
FORM ALV_CATALOG_HEADER.
  ...
  LS_LINE-TYP  = 'H'.
  LS_LINE-KEY = ' '.
  LS_LINE-INFO = 'Material Sales Analysis Month Wise Report' .
