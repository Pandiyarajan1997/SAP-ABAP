...
TYPES: BEGIN OF TY_VK11,
  KSCHL TYPE RV13A-KSCHL,
   ALAND  TYPE KOMG-ALAND,
    WKREG  TYPE  KOMG-WKREG,
        REGIO TYPE  KOMG-REGIO,
           TAXK2   TYPE  KOMG-TAXK2,
      MATNR  TYPE  KOMG-MATNR,
*  KBSTAT      TYPE KOMG-KBSTAT,
    KBETR  TYPE  STRING,
...
perform bdc_field       using 'BDC_OKCODE'
                              '=WEIT'.
perform bdc_dynpro      using 'SAPMV13A' '1927'.
perform bdc_field       using 'BDC_CURSOR'
                              'KOMG-TAXK2'.
perform bdc_field       using 'BDC_OKCODE'
                              '/00'.
...
perform bdc_field       using 'KOMG-REGIO'
                              WA_ITAB-REGIO. " 'TN'.
perform bdc_field       using 'KOMG-TAXK2'
                              WA_ITAB-TAXK2. " '1'.
...
if WA_ITAB-KBETR ne 0.
WA_ITAB-KBETR = WA_ITAB-KBETR / 10.
ENDIF.
...
CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
  EXPORTING
    INPUT         = WA_ITAB-KONWA
 IMPORTING
  OUTPUT        =  WA_ITAB-KONWA.
perform bdc_field       using 'KONP-KONWA(01)'
                             WA_ITAB-KONWA.  " '%'.
perform bdc_field       using 'RV13A-DATAB(01)'
                            WA_ITAB-DATAB."  '01.08.2013'.
...
WA_FINAL-STATUS = I_BDCMSG-MSGTYP.
    ...
    CONCATENATE WA_ITAB-MATNR 'Ended with Error' INTO WA_FINAL-MESSAGE SEPARATED BY SPACE."#EC CI_FLDEXT_OK[2215424] "Added by SPLABAP during code remediation
...
WA_FINAL-STATUS =  I_BDCMSG-MSGTYP.
    CONCATENATE WA_ITAB-MATNR 'Saved Successfully' INTO WA_FINAL-MESSAGE SEPARATED BY SPACE."#EC CI_FLDEXT_OK[2215424] "Added by SPLABAP during code remediation
WA_FINAL-FIELD = I_BDCMSG-MSGV1.
WA_FINAL-FIELD1 = I_BDCMSG-MSGV2.
    ENDIF.
