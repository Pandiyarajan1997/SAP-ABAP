...
FORM get_select_data .
  ...
  IF p_rad4 IS NOT INITIAL.
    ...
    IF s_vbeln[]  IS INITIAL.
      ...
      RETURN.
    ELSE.
      SELECT docno UP TO 1 ROWS FROM zdms_invoice_irn INTO v_refnum WHERE docno IN s_vbeln ORDER BY PRIMARY KEY.
  ...
  ENDIF.
  LOOP AT s_werks.
    IF s_werks-low(1) NE 'D'.
      MESSAGE 'Plant is not belongs to DMS Company code' TYPE 'S' DISPLAY LIKE 'E'.
      LEAVE LIST-PROCESSING .
    ENDIF.
  ENDLOOP.
  IF s_werks[] IS INITIAL.
    SELECT bwkey FROM  t001k INTO TABLE @DATA(lt_werks) WHERE bukrs = 'DMS1'.
    LOOP AT lt_werks INTO DATA(lw_werks).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = lw_werks-bwkey ) TO s_werks.
    ENDLOOP.
  ENDIF.
  ...
  IF p_rad2 IS NOT INITIAL.   "For Cancel IRN/ Eway Bill Invoice Details
    ...
    SELECT a~vbeln "#EC CI_DB_OPERATION_OK[2768887] " Added by <IT-CAR Tool> during Code Remediation
           ...
           c~status            "Eway bill Status
                FROM vbrk AS a
*                INNER JOIN vbrp AS d ON d~vbeln = a~vbeln
                INNER JOIN zdms_invoice_irn AS b
                ON b~docno = a~vbeln AND b~irn_status = 'ACT'
                LEFT OUTER JOIN zdms_invoice_ewb AS c
                ON c~docno = a~vbeln            "
                INTO TABLE l_tab_vbrk
                WHERE a~vbeln IN s_vbeln
                 AND a~fkart IN ('YDMS','YRMS')       "Cancelled Invoice
                AND   a~vkorg = 'SDMS'
                AND   a~fksto = ' '
...
*************check e-invoice*************
*        SELECT SINGLE check_type FROM zsd_re_statu_dms INTO @DATA(ls_check)
*                                                       WHERE distributor = @l_wrk_vbrk-distrb
*                                                       AND   check_type  = '12'
*                                                       AND   check_act   = @abap_true.
...
*                AND   d~werks IN s_werks.
    ...
    LOOP AT l_tab_vbrk INTO l_wrk_vbrk.
*************check e-invoice*************
*      SELECT SINGLE check_type FROM zsd_re_statu_dms INTO @ls_check
*                                                     WHERE distributor = @l_wrk_vbrk-distrb
*                                                     AND   check_type  = '12'
*                                                     AND   check_act   = @abap_true.
...
*                on d~vbeln = a~vbeln and d~werks = p_werks
                LEFT JOIN zdms_invoice_irn AS b
                ON b~docno = a~vbeln
                LEFT OUTER JOIN zdms_invoice_ewb AS c
                ON c~docno = a~vbeln
                INTO TABLE l_tab_vbrk
...
*************check e-invoice*************
        SELECT SINGLE a~kunnr
          INTO l_wrk_vbrk-distrb
          FROM kna1 AS a
          INNER JOIN vbrp AS b ON a~werks = b~vkbur
          WHERE b~vbeln = l_wrk_vbrk-vbeln.
  ...
  ELSEIF p_rad4 IS NOT INITIAL.            "For Eway Bill Create for IRN Created Invoice
    ...
    SELECT a~vbeln "#EC CI_DB_OPERATION_OK[2768887] " Added by <IT-CAR Tool> during Code Remediation
           ...
           c~status
                ...
                AND   a~erdat IN s_erdat
                AND   a~fksto = ' '
                AND   a~fkart  IN ('YDMS','YRMS').      "Restricted Service



*****Removing the Active Eway Bill ********
    DATA: wa_ewb TYPE zdms_invoice_ewb.

    LOOP AT l_tab_vbrk INTO l_wrk_vbrk.
      SELECT * UP TO 1 ROWS FROM zdms_invoice_ewb INTO wa_ewb
...
*                                                     AND   check_act   = @abap_true.
  ...
  LOOP AT lt_einv_final ASSIGNING FIELD-SYMBOL(<fs_f1>).
    SELECT SINGLE werks
      FROM vbrp INTO <fs_f1>-werks
      WHERE vbeln =  <fs_f1>-vbeln.
    SELECT SINGLE kunnr
                  name1
      FROM kna1 INTO (<fs_f1>-distrb,
                      <fs_f1>-distrb_t)
      WHERE werks =  <fs_f1>-werks.
    SELECT SINGLE name1
      FROM kna1 INTO  <fs_f1>-kunag_t
      WHERE kunnr =  <fs_f1>-kunag.
  ENDLOOP.
...
*  gs_fieldcat-no_out = 'X'. " Do not Display Column
  ...
  CLEAR gs_fieldcat.
  gs_fieldcat-fieldname = 'DISTRB'.
  gs_fieldcat-tabname = 'VBRK'.
  gs_fieldcat-col_pos = 6.
  gs_fieldcat-coltext = 'Distributor Number'.
*  gs_fieldcat-no_out = 'X'. " Do not Display Column
  INSERT gs_fieldcat INTO TABLE gt_fieldcat.
  CLEAR gs_fieldcat.
  gs_fieldcat-fieldname = 'DISTRB_T'.
  gs_fieldcat-tabname = 'VBRK'.
  gs_fieldcat-col_pos = 7.
  gs_fieldcat-coltext = 'Distributor Name'.
*  gs_fieldcat-no_out = 'X'. " Do not Display Column
  INSERT gs_fieldcat INTO TABLE gt_fieldcat.
  CLEAR gs_fieldcat.
  gs_fieldcat-fieldname = 'WERKS'.
  gs_fieldcat-tabname = 'VBRP'.
...
*  gs_fieldcat-no_out = 'X'. " Do not Display Column
  ...
  IF p_rad2 = abap_false .
    ...
    gs_fieldcat-tabname     = 'ZDMS_INVOICE_IRN'.
...
*  gs_fieldcat-no_out = 'X'. " Do not Display Column
  ...
  IF p_rad2 IS NOT INITIAL OR p_rad3 IS NOT INITIAL OR p_rad4 IS NOT INITIAL. "Create Ewaybill / Cancel Ewaybill / Display
    ...
    gs_fieldcat-tabname = 'ZDMS_INVOICE_EWB'.
