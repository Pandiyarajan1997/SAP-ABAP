class ZCL_API_DIRECT_SALESORDER_DMS definition
  public
  final
  create public .
...
CLASS ZCL_API_DIRECT_SALESORDER_DMS IMPLEMENTATION.
...
***********pricing condition base on company code*************
        IF lw_sale_hdr-bukrs = 'DMS1'.
          DATA(lv_price) = 'ZPR0'.
        ELSEIF lw_sale_hdr-bukrs = 'ALPN'.
          lv_price = 'PRAP'.
*        ELSEIF lw_sale_hdr-bukrs = 'F001' OR lw_sale_hdr-bukrs = 'F006'
*            OR lw_sale_hdr-bukrs = 'F002' OR lw_sale_hdr-bukrs = 'F003'
*            OR lw_sale_hdr-bukrs = 'F004' OR lw_sale_hdr-bukrs = 'F005'.
        ELSEIF lw_sale_hdr-auart = 'YFEC'.
          lv_price = 'ZMRP'.
        ENDIF.
...
*********************only for franchise company code*****************
*          IF lw_sale_hdr-bukrs = 'F001' OR lw_sale_hdr-bukrs = 'F002' OR lw_sale_hdr-bukrs = 'F006'
*          OR lw_sale_hdr-bukrs = 'F003' OR lw_sale_hdr-bukrs = 'F004' OR lw_sale_hdr-bukrs = 'F005'.
          IF lw_sale_hdr-auart = 'YFEC'.
** *********************ZMRP checking*************
            SELECT SINGLE a~matnr,b~loevm_ko FROM a925 AS a
                                   INNER JOIN konp AS b ON a~knumh = b~knumh
                                   INTO @DATA(ls_zpr0)
                                   WHERE a~kschl = @lv_price
                                   AND   a~auart_sd = @lw_sale_hdr-auart
                                   AND   a~matnr = @lw_sale_itm-matnr
                                   AND   a~datbi GE @sy-datum
                                   AND   b~loevm_ko = @abap_false.
            IF sy-subrc = 0.
              CLEAR : lw_data-zmrp,lw_data-zmrp_unit.
            ELSE.
              IF lw_data-zmrp IS INITIAL..
                lv_msg = |Material - { lw_sale_itm-matnr } ZMRP input price is missing , { lv_msg }|.
              ELSE.
                IF lw_data-zmrp_unit IS INITIAL.
                  lv_msg = |Material - { lw_sale_itm-matnr } ZMRP UNIT is missing , { lv_msg }|.
                ENDIF.
              ENDIF.
            ENDIF.
          ELSE.
** *********************ZPR0 checking*************
          ...
          IF lw_sale_hdr-auart = 'YFEC'.
            MODIFY gs_input-item FROM lw_data.
          ENDIF.
        ...
        IF lv_msg IS INITIAL.
          ...
          CALL FUNCTION 'ZSD_SPL_SALES_ORDER_CREATE_DMS'
            EXPORTING
