  ...
  IF GT_PBIM IS NOT INITIAL.
    ...
    SELECT
           LABST
            ...
            WERKS
            LGORT
         FROM MARD INTO TABLE GT_MARD1 FOR ALL ENTRIES IN GT_PBIM WHERE MATNR  = GT_PBIM-MATNR AND WERKS  = '1100'.
    "1151 PLANT
    ...
    SELECT
            ...
            WERKS
            LGORT
         FROM MARD INTO TABLE GT_MARD2 FOR ALL ENTRIES IN GT_PBIM WHERE MATNR  = GT_PBIM-MATNR AND WERKS = '1151'.
    "1126 PLANT
...
*
    ...
    SELECT
          ...
          MATNR
          WERKS
      FROM MARC INTO TABLE GT_MARC1 FOR ALL ENTRIES IN GT_PBIM WHERE MATNR = GT_PBIM-MATNR AND WERKS ='1100'.
...
FROM MARA INTO TABLE GT_MARA1 FOR ALL ENTRIES IN GT_MARD5 WHERE MATNR = GT_MARD5-MATNR .
   ...
   SELECT
    ...
    SELECT
         ...
         SOBSL
      FROM
       MARC INTO TABLE GT_MARCSP6 FOR ALL ENTRIES IN GT_MARD5 WHERE MATNR = GT_MARD5-MATNR AND WERKS = '1100'.
...
*     ENDIF.
  ...
  LOOP AT GT_FINAL INTO WA_FINAL.
    ...
    LOOP AT GT_MARD1 INTO WA_MARD1 WHERE MATNR = WA_FINAL-MATNR1 AND WERKS = '1100'.
      IF SY-SUBRC = 0.
        WA_FINAL-LABST1 =  WA_FINAL-LABST1 + WA_MARD1-LABST + WA_MARD1-INSME.
      ENDIF.
    ENDLOOP.

    READ TABLE GT_MARC1 INTO WA_MARC1 WITH KEY MATNR = WA_FINAL-MATNR1 WERKS = '1100' .
    IF SY-SUBRC = 0.
      WA_FINAL-TRAME1 =  WA_FINAL-TRAME1 + WA_MARC1-TRAME.
    ENDIF.

    LOOP AT GT_MARD2 INTO WA_MARD2 WHERE MATNR = WA_FINAL-MATNR1 AND WERKS = '1151'.
      IF SY-SUBRC = 0.
    ...
    READ TABLE GT_MARC2 INTO WA_MARC2 WITH KEY MATNR = WA_FINAL-MATNR1  WERKS = '1151' .
    ...
    READ TABLE GT_MARC3 INTO WA_MARC3 WITH KEY MATNR = WA_FINAL-MATNR1  WERKS = '1126' .
    IF SY-SUBRC = 0.
    ...
    READ TABLE GT_MARC4 INTO WA_MARC4 WITH KEY MATNR = WA_FINAL-MATNR1  WERKS = '1176' .
    IF SY-SUBRC = 0.
...
**********************************REMOVE DUPLICATE*******************************
  ...
  LOOP AT GT_FINAL INTO WA_FINAL.
    IF SY-SUBRC = 0.
      IF WA_FINAL-WERKS NE 1126  AND WA_FINAL-WERKS NE 1127 AND WA_FINAL-WERKS NE 1128 AND WA_FINAL-WERKS NE 1129 AND WA_FINAL-WERKS NE 1130 AND WA_FINAL-WERKS NE 1131 AND
        WA_FINAL-WERKS NE 1132 AND WA_FINAL-WERKS NE 1133 AND WA_FINAL-WERKS NE 1134.
        WA_FINAL-STKGHW = ' '.
        WA_FINAL-STKTRNGHW = ' '.
        WA_FINAL-GHMATNR = ' '.
  ...
  LOOP AT GT_FINAL INTO WA_FINAL.
    IF SY-SUBRC = 0.
      IF WA_FINAL-WERKS NE 1176 AND WA_FINAL-WERKS NE 1177 AND WA_FINAL-WERKS NE 1178 AND WA_FINAL-WERKS NE 1179 .
        WA_FINAL-STKMUW = ' '.
        WA_FINAL-STKTRNMUW = ' '.
        WA_FINAL-MUMATNR  = ' '.
...
*******************************************End 1176********************************
  ...
  LOOP AT GT_FINAL INTO WA_FINAL.
    ...
    UPWHC_QTY1  = WA_FINAL-FINALQTY1 -  ( WA_FINAL-STKCUW + WA_FINAL-STKTRNCUW ).
...
***********************************************************End of All WHC Calucations*****************************
  ...
  LOOP AT GT_FINAL INTO WA_FINAL.
    UPTOWHC_QTY  = WA_FINAL-FINALQTY1 -  ( WA_FINAL-LABST1 + WA_FINAL-TRAME1 + WA_FINAL-STKCUW + WA_FINAL-STKTRNCUW ).
        ...
        ENDLOOP.
           ...
           READ TABLE GT_UNIT1TRN INTO WA_UNIT1TRN WITH KEY MATNR = WA_FINAL-MATNR1 WERKS = '1001' .
            IF SY-SUBRC = 0.
             WA_FINAL-UNIT1TR = WA_UNIT1TRN-TRAME.
              ENDIF.
            P1_MRPQTY  = WA_FINAL-FINALQTY1 -  ( WA_FINAL-LABST1 + WA_FINAL-TRAME1 + WA_FINAL-STKCUW + WA_FINAL-STKTRNCUW ).
...
*
  ...
  SORT GT_FINAL DESCENDING BY MATNR3 WERKS3 NAME1 BEZEI LABST .
...
SORT GT_FINAL DESCENDING BY WERKS .
...
FORM FIELDCATLOG.
  ...
  PERFORM ALV_LAYOUT USING 2 'Without.Req.Plant' 'WERKS3' 'GT_FINAL' '' '' 'X' '' '' '' .
  ...
  PERFORM ALV_LAYOUT USING 12 'Req Plan' 'PBDNR' 'GT_FINAL' '' '' '' ''  '' ''.
...
*  PERFORM ALV_LAYOUT USING 28 'Req.Final Qty@Branch Level' 'FINALQTY' 'GT_FINAL' '' '' '' '' 'X' ''. " Commedned By Govind
*  PERFORM ALV_LAYOUT USING 30 'Req@Sin.Mat.Code' 'MATNR1' 'GT_FINAL' '' '' '' '' 'X' ''. " Commedned By Govind
  PERFORM ALV_LAYOUT USING 32 'Fin.Req Qty@Branch Wise' 'FINALQTY1' 'GT_FINAL' 'X' 'X' '' '' 'X' ''.
...
*  PERFORM ALV_LAYOUT USING 36 'Stock in Transit@1126' 'STKTRNGHW' 'GT_FINAL' 'X' '' '' ''  'X' ''. " Commedned By Govind
  PERFORM ALV_LAYOUT USING 38 'Closing Stock in 1151(CUTTACK RWH)' 'STKCUW' 'GT_FINAL' 'X' '' '' ''  'X' ''.
  PERFORM ALV_LAYOUT USING 40 'In Transit@1151' 'STKTRNCUW' 'GT_FINAL' 'X' '' '' ''  'X' ''.
...
*  PERFORM ALV_LAYOUT USING 54 'Plan Qty(1st Cycle Cum. in Ltrs)' 'TOTALAMT1' 'GT_FINAL' '' '' '' ''  'X' ''.
  ...
  PERFORM ALV_LAYOUT USING 56 'Fin.Req Qty@1100' 'WHCFINALQTY' 'GT_FINAL' 'X' 'X' '' ''  'X' 'X'.
