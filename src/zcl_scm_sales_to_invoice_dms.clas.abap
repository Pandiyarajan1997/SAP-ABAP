class ZCL_SCM_SALES_TO_INVOICE_DMS definition
  public
  final
  create public .
...
CLASS ZCL_SCM_SALES_TO_INVOICE_DMS IMPLEMENTATION.
  ...
  METHOD create_ob_delivery.
    ...
    IF sales_document IS NOT INITIAL.
      ...
      IF sy-subrc = 0.
        SELECT SINGLE vbeln FROM lips INTO @DATA(lv_delivery) WHERE vgbel = @sales_document.
        IF sy-subrc = 0.
        ...
        LOOP AT lt_items ASSIGNING FIELD-SYMBOL(<fs_items>).
          ...
          IF sy-subrc NE 0.
            DATA(l_msg) = |Kindly check auto batch Determination for Material { <fs_items>-matnr }|.
            EXIT.
          ELSE.
            SELECT SINGLE * FROM kondh INTO @DATA(l_batchsrch) WHERE knumh = @l_autobatch-knumh
                                                               AND chvsk   = 'O'.
            IF sy-subrc NE 0.
              l_msg = |Kindly check auto batch selection strategy for Material { <fs_items>-matnr }|.
              EXIT.
            ENDIF.
          ENDIF.
