FUNCTION zbapi_cust_open_item_dms_nsd.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* to select the rounding off credit and debit entries created manually on 31st march
    ...
    IF lt_customer[] IS NOT INITIAL.
      ...
      SELECT kunnr,dist_werks
        FROM zcust_blk_chk
        INTO TABLE @DATA(lt_kunnrchk)
        FOR ALL ENTRIES IN @lt_customer
        WHERE bukrs = 'DMS1'
          AND vkorg = 'SDMS'
          AND kunnr = @lt_customer-kunnr.
      IF sy-subrc = 0.
        SORT : lt_kunnrchk BY kunnr dist_werks.
      ENDIF.
*          AND block NE 'X'
*          AND dist_werks = @ls_distributor-werks.

**********************get the open item customers*****************

      SELECT a~kunnr,b~rbusa
...
**********************get the cleared item customers*****************
...
*       AND c~block NE 'X'.
    ...
    ELSE.
      ...
      REFRESH : lt_kunnr,lt_kunnrchk.

      SELECT kunnr,dist_werks
        FROM zcust_blk_chk
        INTO TABLE @lt_kunnrchk
        WHERE bukrs = 'DMS1'
          AND vkorg = 'SDMS'.
      IF sy-subrc = 0.
        SORT : lt_kunnrchk BY kunnr dist_werks.
...
*******************************bypass the currently mapped distributor*************
      READ TABLE lt_kunnrchk TRANSPORTING NO FIELDS WITH KEY kunnr = ls_kunnr-kunnr
                                          dist_werks = ls_distributor-werks BINARY SEARCH.
      IF sy-subrc = 0.
        CONTINUE.
      ENDIF.
