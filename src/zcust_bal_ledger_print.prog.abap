...
*********************** Added on 17-10-2014  By Govind ********************************
  IF P_VN  = 'X'.
    SELECT
      ...
      WHERE LIFNR = GT_KNA1-LIFNR AND BUDAT IN SO_BUDAT
         AND BUKRS IN SO_BUKRS AND
            UMSKZ IN SO_UMSKZ  AND UMSKZ <> 'H'  AND BLART IN ('KR','AB','KG') .
    ...
    WHERE LIFNR = GT_KNA1-LIFNR AND BUDAT IN SO_BUDAT
       AND BUKRS IN SO_BUKRS AND
          UMSKZ IN SO_UMSKZ  AND UMSKZ <> 'S'  AND BLART IN ('KA') . " Added By Govind For KA Type on 05-06-2015
   ...
   WHERE LIFNR = GT_KNA1-LIFNR AND BUDAT BETWEEN '01.04.2014'  AND  SO_BUDAT-LOW
    ...
    SELECT
         ...
         WHERE LIFNR = GT_KNA1-LIFNR AND BUDAT BETWEEN '01.04.2014'  AND  SO_BUDAT-LOW
           AND BUKRS IN SO_BUKRS AND
               UMSKZ IN SO_UMSKZ  AND UMSKZ <> 'S' AND BLART IN ('KA') .
 ...
 WHERE LIFNR = GT_KNA1-LIFNR AND BUDAT BETWEEN '01.04.2014'  AND  SO_BUDAT-LOW
   AND BUKRS IN SO_BUKRS AND
    ...
    APPEND LINES OF GT_BSAK3 TO GT_BSID1.
...
*     USER_SETTINGS              = 'X'
      BAL_DATE                   = B_DATE
      LV_OPN                     = LV_OPN
      LV_FRDT                    = SO_BUDAT-LOW "LV_FRDAT
      LV_TODT                    = SO_BUDAT-HIGH "LV_TODAT
      LV_BUK                   = LV_BUK
*   IMPORTING
