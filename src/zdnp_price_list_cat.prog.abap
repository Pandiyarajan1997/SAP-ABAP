...
*        KNUMH TYPE A936-KNUMH,
        ...
        KAPPL TYPE A306-KAPPL,
        KSCHL TYPE A306-KSCHL,
        VKORG TYPE A306-VKORG,
        VTWEG TYPE A306-VTWEG,
        PLTYP TYPE A306-PLTYP,
        WAERK TYPE A306-WAERK,
        MATNR TYPE A306-MATNR,
        KFRST TYPE A306-KFRST,
        DATBI TYPE A306-DATBI,
        DATAB TYPE A306-DATAB,
        KBSTAT TYPE A306-KBSTAT,
        KNUMH TYPE A306-KNUMH,
...
DATA : LV_VKORG TYPE A306-VKORG,
      LV_VTWEG TYPE A306-VTWEG,
      LV_PLTYP TYPE A306-PLTYP,
      LV_WAERK TYPE A306-WAERK,
      LV_BEDAT TYPE A306-DATBI,
      LV_MATNR TYPE A306-MATNR .


TYPES: BEGIN OF STR_A306,
        KAPPL TYPE A306-KAPPL,
        KSCHL TYPE A306-KSCHL,
        VKORG TYPE A306-VKORG,
        VTWEG TYPE A306-VTWEG,
...
DATA:WA_A306 TYPE STR_A306,
      IT_A306 TYPE TABLE OF STR_A306.
...
SELECT-OPTIONS : SO_VKORG FOR LV_VKORG OBLIGATORY,
                 ...
                 SO_PLTYP FOR LV_PLTYP NO-EXTENSION NO INTERVALS OBLIGATORY,
                 SO_WAERK FOR LV_WAERK NO-EXTENSION NO INTERVALS OBLIGATORY,
                 SO_BEDAT FOR LV_BEDAT NO-EXTENSION NO INTERVALS OBLIGATORY,
...
*       KNUMH FROM   A936 INTO CORRESPONDING FIELDS OF TABLE GT_A936 WHERE  VKORG IN SO_VKORG AND WERKS IN SO_WERKS AND MATNR IN SO_MATNR .
  ...
  SELECT  KAPPL
          ...
          DATAB
          KBSTAT
          KNUMH  FROM   A306 INTO CORRESPONDING FIELDS OF TABLE IT_A306 WHERE  VKORG IN SO_VKORG  AND MATNR IN SO_MATNR AND VTWEG IN SO_VTWEG AND
    WAERK IN SO_WAERK AND PLTYP IN SO_PLTYP AND DATBI > SO_BEDAT-LOW.


  SORT IT_A306 BY KNUMH  DESCENDING .

  DELETE ADJACENT DUPLICATES FROM IT_A306 COMPARING  MATNR.



  IF NOT IT_A306[] IS INITIAL.

    SELECT KNUMH
    KSCHL
    KBETR
    KMEIN   FROM KONP INTO CORRESPONDING FIELDS OF TABLE GT_KONP FOR ALL ENTRIES IN IT_A306 WHERE KNUMH = IT_A306-KNUMH  .

    SELECT MATNR
      MAKTX FROM MAKT INTO TABLE GT_MAKT FOR ALL ENTRIES IN IT_A306 WHERE MATNR = IT_A306-MATNR.

    SELECT MATNR
      MEINH
      UMREN FROM MARM INTO TABLE GT_MARM FOR ALL ENTRIES IN IT_A306 WHERE MATNR = IT_A306-MATNR . " AND   MEINH = 'BOT'.
  ENDIF.


  SORT IT_A306 BY MATNR KNUMH.
  ...
  " BREAK-POINT .
  LOOP AT IT_A306 INTO WA_A306 .
    MOVE-CORRESPONDING WA_A306 TO WA_FINAL.
    READ TABLE GT_KONP INTO WA_KONP WITH KEY  KNUMH = WA_A306-KNUMH BINARY SEARCH.
    WA_FINAL-KBETR = WA_KONP-KBETR.
    WA_FINAL-KMEIN = WA_KONP-KMEIN.
    READ TABLE GT_MAKT INTO WA_MAKT WITH KEY MATNR = WA_A306-MATNR BINARY SEARCH.
    WA_FINAL-MAKTX = WA_MAKT-MAKTX.
    LOOP AT GT_MARM INTO WA_MARM WHERE MATNR = WA_A306-MATNR .
...
FORM ALV_FIELDCAT .
  ...
  CLEAR  WA_FIELDCAT.
    ...
    WA_FIELDCAT-FIELDNAME   = 'PLTYP'.
  WA_FIELDCAT-SELTEXT_M   = 'Price List'.
