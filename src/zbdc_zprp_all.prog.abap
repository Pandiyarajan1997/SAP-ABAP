...
PARAMETERS: PS_PARM AS LISTBOX VISIBLE LENGTH 20 USER-COMMAND ABC MODIF ID TB1.            " SELECTION SCREEN PARAMETER FOR INVOICE AND CUSTOMER BALANCES PS_PARM
...
INITIALIZATION.
  ...
  GWA_LIST-KEY = '2'.
  GWA_LIST-TEXT = 'Sales Office/Material'.
  APPEND GWA_LIST TO GT_LIST.
  CLEAR: GWA_LIST.

  GWA_LIST-KEY = '3'.
  GWA_LIST-TEXT = 'Cust.group/Material'.
  APPEND GWA_LIST TO GT_LIST.
  CLEAR: GWA_LIST.
...
AT SELECTION-SCREEN.
  ...
  IF GWA_VALUES-FIELDVALUE = 1.
    ...
    CALL TRANSACTION 'ZPRP_CUST'.

  ENDIF.

  IF GWA_VALUES-FIELDVALUE = 2.

    CALL TRANSACTION 'ZPRP_SALE'.

  ENDIF.

  IF GWA_VALUES-FIELDVALUE = 3.

    CALL TRANSACTION 'ZPRP_REG'.
