...
DATA : LV_VKORG TYPE A936-VKORG,
       LV_VTWEG TYPE A936-VTWEG,
      LV_WERKS TYPE A936-WERKS,
      LV_BEDAT TYPE A936-DATBI,
      LV_MATNR TYPE A936-MATNR .
...
FORM GET_DATA .
  SELECT  KSCHL
    ...
    AND KSCHL = 'PR00' AND VTWEG IN SO_VTWEG AND DATBI > SO_BEDAT-LOW .


SORT GT_A936 BY KNUMH  DESCENDING .

  DELETE ADJACENT DUPLICATES FROM GT_A936 COMPARING  MATNR.

  IF NOT GT_A936[] IS INITIAL.

    SELECT KNUMH
    KSCHL
    KBETR
    KMEIN   FROM KONP INTO CORRESPONDING FIELDS OF TABLE GT_KONP FOR ALL ENTRIES IN GT_A936 WHERE KNUMH = GT_A936-KNUMH  .

    SELECT MATNR
      MAKTX FROM MAKT INTO TABLE GT_MAKT FOR ALL ENTRIES IN GT_A936 WHERE MATNR = GT_A936-MATNR.

    SELECT MATNR
      MEINH
      UMREN FROM MARM INTO TABLE GT_MARM FOR ALL ENTRIES IN GT_A936 WHERE MATNR = GT_A936-MATNR . " AND   MEINH = 'BOT'.
  ENDIF.


  SORT GT_A936 BY MATNR KNUMH.
 ...
 " BREAK-POINT .
  LOOP AT GT_A936 INTO WA_A936 .
    MOVE-CORRESPONDING WA_A936 TO WA_FINAL.
    READ TABLE GT_KONP INTO WA_KONP WITH KEY  KNUMH = WA_A936-KNUMH BINARY SEARCH.
    WA_FINAL-KBETR = WA_KONP-KBETR.
    WA_FINAL-KMEIN = WA_KONP-KMEIN.
    READ TABLE GT_MAKT INTO WA_MAKT WITH KEY MATNR = WA_A936-MATNR BINARY SEARCH.
    WA_FINAL-MAKTX = WA_MAKT-MAKTX.
    LOOP AT GT_MARM INTO WA_MARM WHERE MATNR = WA_A936-MATNR .
...
*  ENDLOOP .
  ...
  SORT GT_FINAL BY MATNR WERKS KNUMH DESCENDING .
