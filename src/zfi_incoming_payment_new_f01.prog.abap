...
*      PERFORM progress_indicator USING 'Posting and clearing' 'for Customer:' gs_tab-kunnr 'is in progress'.
    ...
    CLEAR: gv_partial, gv_excess, gv_exact, gv_wrbtr, gv_wrbtr1.

*Perform F-32 once more after Payment adjustment to take the Balance DZ and AB -ve Amounts to clear possible payments
* F-32 for other credit notes/excess payments/un-addresed partial payment docs.
    IF gs_tab-belnr IS INITIAL .
* get open items for customer
      FREE: gs_bapireturn, gt_lineitems, gt_openitems, gt_openitemsc, gt_amnt, gv_wrbtr, gv_wrbtrc, gv_wrbtro, gt_fcat, gt_cusven, gt_lineitemsv.
...
*  ENDIF.
  ...
  ENDLOOP.
  SELECT SINGLE * FROM  zfi_cust_inpymt
    INTO @DATA(ls_table1)
    WHERE bukrs = @bukrs
    AND kunnr = @kunnr
    AND xblnr = @xblnr
  ...
  IF sy-subrc = 0.
    ...
    ELSE.
      ...
      ENDIF.
    ENDIF.
    MODIFY zfi_cust_inpymt FROM ls_table1.
  ENDIF.
ENDFORM.
*&---------------------------------------------------------------------*
...
*  SORT gt_openitemsc BY due_date cpudt ASCENDING cputm ASCENDING.
  SORT gt_openitemsc BY pstng_date cpudt ASCENDING cputm ASCENDING.

*  SORT pt_openitems BY  root_year root_doc due_date DESCENDING cpudt ASCENDING cputm ASCENDING item_num DESCENDING.
  SORT pt_openitems BY  root_year root_doc pstng_date DESCENDING cpudt ASCENDING cputm ASCENDING item_num DESCENDING.
...
*    ENDIF.
    ...
    IF lv_due_date IS INITIAL.
      lv_due_date = gs_openitems-pstng_date.
    ELSEIF lv_due_date GT gs_openitems-pstng_date.
      lv_due_date = gs_openitems-pstng_date.
    ENDIF.
...
*  SORT pt_openitems BY  root_year root_doc root_item due_date ASCENDING cpudt ASCENDING cputm ASCENDING item_num ASCENDING.
  SORT pt_openitems BY  root_year root_doc root_item PSTNG_DATE ASCENDING cpudt ASCENDING cputm ASCENDING item_num ASCENDING.
...
* 3rd screen
  ...
  IF sy-subrc IS INITIAL.
    ...
    MODIFY zdebit_ref_mis FROM ls_table1.
