FUNCTION ZGET_DATA_MIS_STK_ROL.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  TABLES
*"      IT_STOCK_ROL STRUCTURE  ZSTR_STK_ROL_MIS
*"----------------------------------------------------------------------
...
SELECT
    ...
    MTART
    MATKL
    MEINS FROM MARA INTO TABLE IT_MARA WHERE mtart in ('FERT','HAWA').
  ...
  SELECT
    ...
    EISBE
    TRAME FROM MARC
INTO TABLE IT_MARC FOR ALL ENTRIES IN IT_MARA WHERE matnr = IT_MARA-matnr AND PSTAT <> 'X' AND DISMM <> 'ND' AND MINBE <> '0'.
...
UMREN  UMREZ FROM MARM INTO TABLE IT_MARM FOR ALL ENTRIES IN IT_MARC WHERE MATNR = IT_MARC-MATNR .
  ...
  LOOP AT IT_MARC INTO WA_MARC.
    IT_STOCK_ROL-MATNR = WA_MARC-MATNR.
    IT_STOCK_ROL-WERKS = WA_MARC-WERKS.
    IT_STOCK_ROL-MINBE = WA_MARC-MINBE.
    IT_STOCK_ROL-EISBE = WA_MARC-EISBE.
    IT_STOCK_ROL-TRAME = WA_MARC-TRAME.
    READ TABLE IT_MBEW INTO WA_MBEW WITH KEY MATNR = WA_MARC-MATNR.
    IT_STOCK_ROL-VERPR = WA_MBEW-VERPR.
    IT_STOCK_ROL-STPRS = WA_MBEW-STPRS.
    IT_STOCK_ROL-LBKUM = WA_MBEW-LBKUM.
    READ TABLE IT_MARM INTO WA_MARM WITH KEY MATNR = WA_MARC-MATNR.
    IT_STOCK_ROL-UMREN = WA_MARM-UMREN.
    IT_STOCK_ROL-UMREZ = WA_MARM-UMREZ.
    READ TABLE IT_MAKT INTO WA_MAKT WITH KEY MATNR = WA_MARC-MATNR.
    IT_STOCK_ROL-MAKTG = WA_MAKT-MAKTG.
    READ TABLE IT_MARA INTO WA_MARA WITH KEY MATNR = WA_MARC-MATNR.
    IT_STOCK_ROL-MTART = WA_MARA-MTART.
    IT_STOCK_ROL-MATKL = WA_MARA-MATKL.
    IT_STOCK_ROL-MEINS = WA_MARA-MEINS.
    APPEND IT_STOCK_ROL.
