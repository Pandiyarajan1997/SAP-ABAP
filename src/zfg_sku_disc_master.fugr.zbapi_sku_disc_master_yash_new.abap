FUNCTION ZBAPI_SKU_DISC_MASTER_YASH_NEW .
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
if S_DATE[] is INITIAL.
    SELECT OBJECTID from CDHDR INTO TABLE it_CDHDR WHERE OBJECTCLAS = 'COND_A' and UDATE = sy-datum.
else.
  SELECT OBJECTID from CDHDR INTO TABLE it_CDHDR WHERE OBJECTCLAS = 'COND_A' and UDATE in s_date.
endif.
...
if it_cdhdr[] is NOT INITIAL.
    SELECT KAPPL
       ...
       and KNUMH = IT_CDHDR-knumh ORDER BY PRIMARY KEY.  " Added by <IT-CAR Tool> during Code Remediation
...
if it_cdhdr[] is not INITIAL.
    SELECT A~KAPPL
      ...
      FOR ALL ENTRIES IN it_cdhdr "IT_MARA
      WHERE A~KAPPL = 'V'
       AND  A~KSCHL IN ('Z004', 'Z005', 'Z006', 'ZPRB', 'ZBSD')
      AND   A~KNUMH = it_cdhdr-KNUMH.  "A~MATNR  = IT_MARA-MATNR.
