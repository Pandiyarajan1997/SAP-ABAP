FUNCTION zauto_clearing_for_vendor_inv.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(COMP_CODE) TYPE  BUKRS
*"     REFERENCE(VENDOR) TYPE  LIFNR
*"     REFERENCE(ORG_INV_NO) TYPE  BELNR_D
*"     REFERENCE(AMOUNT) TYPE  WRBTR
*"     REFERENCE(INV_REFNO) TYPE  XBLNR1
*"     VALUE(PAYMENT_REFERENCE) TYPE  ZPAY_REF
*"     REFERENCE(ASSIGNMENT) TYPE  CHAR25
*"     REFERENCE(DOC_DATE) TYPE  BLDAT
*"     REFERENCE(POSTING_DATE) TYPE  BUDAT
*"     REFERENCE(GLACCOUNT) TYPE  SAKNR
*"     REFERENCE(ADV_TYPE) TYPE  DZUMSK OPTIONAL
*"     REFERENCE(ADV_TEXT) TYPE  SGTXT OPTIONAL
*"  EXPORTING
*"     REFERENCE(CLEARDOCUMENT_NO) TYPE  BELNR_D
*"     REFERENCE(MSG_TYP) TYPE  CHAR01
*"     REFERENCE(MESSAGE) TYPE  CHAR50
*"     REFERENCE(ACTUAL_DOCUMENT) TYPE  ZFI_DOC_TT
*"     REFERENCE(INT_POSTDOC_NO) TYPE  BELNR_D
*"  TABLES
...
** Purpose: It is for posting Clearing Documents for Vendor Invoice Payments
**            based on MIS input **
*"------------------------------------------------------------------------------
...
*** Error Handling in Initial Process ***
  ...
  ELSE.
    ...
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
      ...
      IMPORTING
        output = lv_vendor.
*Added By Samsudeen M on 01.06.2023
    SELECT SINGLE name1 FROM lfa1
      INTO @DATA(lv_venname)
      WHERE lifnr = @lv_vendor.
  ENDIF.

  IF adv_type IS INITIAL.    "New trial Code

    IF org_inv_no IS INITIAL.
      RAISE enter_inv_docno.
    ELSE.
...
******************************************************************************************
    IF adv_type IS INITIAL. "Clearing only required when it is not advance payment
*** Actual Document which is getting Cleared ***
      lt_ftclear = VALUE #( ( agkoa = 'K' agkon = vendor agbuk = comp_code xnops = 'X' selfd = 'BELNR' selvon = org_inv_no ) ).
...
*Changed On: 19.05.2023 *Changed By: Samsudeen M
    ...
    ELSE.
      IF adv_text is INITIAL.
        lv_sgtxt = |Advance Payment for { lv_venname }|.
      ELSE.
        lv_sgtxt = adv_text.
      ENDIF.
    ...
    IF adv_type IS INITIAL.
      "for normal payment with clearing
      lt_ftpost = VALUE #(
                 ( stype = 'K' count = lv_count     fnam = 'BKPF-BUKRS' fval = comp_code )
                 ...
                 ( stype = 'K' count = lv_count     fnam = 'BKPF-BLART' fval = 'KZ' )
                 ( stype = 'K' count = lv_count     fnam = 'BKPF-XBLNR' fval = inv_refno )
                 ( stype = 'P' count = lv_count     fnam = 'RF05A-NEWBS' fval = '50' )
                 ( stype = 'P' count = lv_count     fnam = 'BSEG-HKONT' fval = glaccount )
                 ( stype = 'P' count = lv_count     fnam = 'BSEG-WRBTR' fval = lv_amount )
                 ( stype = 'P' count = lv_count     fnam = 'BSEG-ZUONR' fval = assignment )
                 ( stype = 'P' count = lv_count     fnam = 'BSEG-SGTXT' fval = lv_sgtxt ) ).
    ELSE.
      "for posting advance payment entries without clearing
      lt_ftpost = VALUE #(
               ( stype = 'K' count = lv_count     fnam = 'BKPF-BUKRS' fval = comp_code )
               ( stype = 'K' count = lv_count     fnam = 'BKPF-BLDAT' fval = lv_date )
               ( stype = 'K' count = lv_count     fnam = 'BKPF-BUDAT' fval = lv_date1 )
               ...
               ( stype = 'P' count = lv_count     fnam = 'RF05A-NEWBS' fval = '50' )
               ( stype = 'P' count = lv_count     fnam = 'RF05A-NEWKO' fval = glaccount )
               ( stype = 'P' count = lv_count     fnam = 'BSEG-WRBTR' fval = lv_amount )
               ( stype = 'P' count = lv_count     fnam = 'BSEG-SGTXT' fval = lv_sgtxt )

               ( stype = 'P' count = lv_count2     fnam = 'RF05A-NEWBS' fval = '29' )   "'50'
               ( stype = 'P' count = lv_count2     fnam = 'RF05A-NEWKO' fval = vendor )
               ( stype = 'P' count = lv_count2     fnam = 'RF05A-NEWUM' fval = adv_type )
               ( stype = 'P' count = lv_count2     fnam = 'BSEG-WRBTR' fval = lv_amount )
               ( stype = 'P' count = lv_count2     fnam = 'BSEG-SGTXT' fval = lv_sgtxt )
               ( stype = 'P' count = lv_count2     fnam = 'BSEG-ZFBDT' fval = lv_date1 )
               ( stype = 'P' count = lv_count2     fnam = 'BSEG-MWSKZ' fval = 'I0' ) ).
    ENDIF.
...
*** Actual Call for Clearing and Posting ****
    ...
    IF sy-subrc = 0.
      IF lv_msgid = 'F5' AND lv_msgnr = '312' AND lv_msgty = 'S'.
        ...
        IF cleardocument_no IS NOT INITIAL and adv_type IS INITIAL.
